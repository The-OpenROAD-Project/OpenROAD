...
| `-sink_clustering_size` | Specifies the maximum number of sinks per cluster for the register tree. The allowed values are integers `[0, MAX_INT]`. If this is not specified, the size is inferred from the sink buffer's maximum fanout, defaulting to `30` if no limit is found, and may vary by PDK and library constraints. |
