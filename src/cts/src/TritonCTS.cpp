...
void TritonCTS::findLongEdges(stt::Tree& clkSteiner, odb::Point driverPt, std::map<odb::Point, std::vector<odb::dbITerm*>>& point2pin)
{
  ...
  int64_t threashold = std::max(dx, dy) / 2;
  logger_->report("treashold (die side / 2) = {}", threashold);
  ...
  for (stt::Branch branch : clkSteiner.branch) {
    ...
    if(clusterFrom != -1 && clusterTo != -1) {
      ...
      clusters[mantainedCLuster].insert(clusters[mantainedCLuster].end(), clusters[removedCLuster].begin(), clusters[removedCLuster].end());
