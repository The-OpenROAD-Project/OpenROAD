/* ===========================================================================
 * Copyright (C) 2003-2006, UW VLSI CAD group.  All rights reserved.
 *
 * This software is NOT "freeware". See the file entitled "LICENSE" (provided
 * in this distribution) for the complete copyright notice and licensing terms.
 * ===========================================================================
 *
 * Description:
 * This file contains code for DetailedMis-like placement of cells.
 */
#include <deque>
#include <set>
#include <vector>
...
public:
	...
	// ************************
	{
		return _xmax >= _xmin;	// (Only need to check x, as y would be redundant)
	}
...
// ********************
{
	inline bool operator() ( const VertexDegree &p, const VertexDegree &q ) const
	// ********************************************************************
	{
...
////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
DetailedMis::DetailedMis( Architecture* arch, Network* network, RoutingParams* rt ):
    m_mgrPtr( 0 ),
    m_arch( arch ),
    m_network( network ),
    m_rt( rt ),
...
/*
	...
	std::fill( _mappingNodeToLocation.begin(), _mappingNodeToLocation.end(), (SwapLocation *) NULL );
 */
}

////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
DetailedMis::~DetailedMis( void )
{
    clearGrid();
}
...
void DetailedMis::run( DetailedMgr* mgrPtr, std::vector<std::string>& args )
{
    ...
    {
        std::cout << boost::format( "Pass %3d of mis; hpwl is %.6e; disp is %.6e\n" ) % p % curr_hpwl % curr_disp;
        ...
        last_hpwl = curr_hpwl;
        curr_hpwl = Utility::hpwl( m_network, hpwl_x, hpwl_y );
        if( m_obj == DetailedMis::Hpwl && std::fabs( curr_hpwl - last_hpwl ) / last_hpwl <= tol )
        {
        ...
        last_disp = curr_disp;
        curr_disp = Utility::disp_l1( m_network, tot_disp, max_disp, avg_disp );
        if( m_obj == DetailedMis::Disp && std::fabs( curr_disp - last_disp ) / last_disp <= tol )
        {
    ...
    m_mgrPtr->resortSegments();
    std::cout << boost::format( "End of passes for mis; hpwl is %.6e, total imp is %.2lf%%; disp is %.6e, total imp is %.2lf%%\n" )
...
void DetailedMis::collectMovableCells( void )
{
    ...
    std::cout << "Number of candidate cells is " << m_candidates.size() << std::endl;
}
...
{
    std::cout << "Coloring the cells (skipping edges > " << m_skipEdgesLargerThanThis << ")" << std::endl;
    ...
    for( int e = 0; e < m_network->m_edges.size(); e++ )
    {
        ...
        int numPins = edi->m_lastPin - edi->m_firstPin;
        if( numPins <= 1 || numPins > m_skipEdgesLargerThanThis )
        {
            continue;
        }

        for( int pi = edi->m_firstPin; pi < edi->m_lastPin; pi++ )
        {
            Pin* pini = m_network->m_edgePins[pi];
            Node* ndi = &(m_network->m_nodes[pini->m_nodeId]);
            if( !m_movable[ndi->getId()] )
            {
                continue;
            }

            for( int pj = pi+1; pj < edi->m_lastPin; pj++ )
            {
...
//        {
//            ++j;
//            std::cout << boost::format( "Color %3d, Cells with this color is %d\n" ) % j % hist[i];
//        }
//    }
}
...
//    std::cout << "Populating grid." << std::endl;
    ...
    for( size_t n = 0; n < m_candidates.size(); ++n ) 
    {
        ...
        int i = std::max( std::min( (int)((ndi->getX() - xmin)/m_stepX), m_dimW-1 ), 0 );
        int j = std::max( std::min( (int)((ndi->getY() - ymin)/m_stepY), m_dimH-1 ), 0 );
    ...
    while( Q.size() > 0 )
    {
        ...
        for( size_t j = 0; j < currPtr->m_nodes.size(); j++ )
        {
            ...
            if( compat )
            {
                ...
                if( m_useSameColor && m_colors[ndi->getId()] != m_colors[ndj->getId()] )
                {
                    compat = false;
                }
            }
            if( compat )
            {
                ...
                if( m_useSameSize && (ndi->getWidth() != ndj->getWidth() || ndi->getHeight() != ndj->getHeight()) )
                {
                    compat = false;
                }
            }
            if( compat )
            {
                ...
                if( spanned_i != spanned_j || ndi->m_powerBot != ndj->m_powerBot || ndi->m_powerTop != ndj->m_powerTop )
                {
                    compat = false;
                }
            }
        ...
        if( currPtr->m_i-1 >= 0 )          Q.push_back( m_grid[currPtr->m_i-1][currPtr->m_j] );
        if( currPtr->m_i+1 <= m_dimW-1 )   Q.push_back( m_grid[currPtr->m_i+1][currPtr->m_j] );
        if( currPtr->m_j-1 >= 0 )          Q.push_back( m_grid[currPtr->m_i][currPtr->m_j-1] );
        if( currPtr->m_j+1 <= m_dimH-1 )   Q.push_back( m_grid[currPtr->m_i][currPtr->m_j+1] );
    }
}
...
void DetailedMis::solveMatch( void )
{
    ...
    std::vector<std::pair<double,double> > pos; // Original location of each node.
    pos.resize( nNodes );
    std::vector<std::vector<DetailedSeg*> > seg; // Original segments for each node.
    seg.resize( nNodes );
    ...
    for( lemon::ListDigraph::ArcMap<int>::ItemIt it(flow); it != lemon::INVALID; ++it )
    {
        ...
        if( g.target(it) != demandNode && g.source(it) != supplyNode && mincost.flow(it) != 0 )
        {
            std::map<lemon::ListDigraph::Arc,std::pair<int,int> >::iterator it1 = reverseMap.find( it );
            ...
            // If cell "i" is assigned to location "i", it means that it has not moved.  
            // We don't need to remove and reinsert it...

            Node* ndi = nodes[i];
            Node* ndj = nodes[j]; // We effectively "moved" into this cells's location.
            int spanned_i = (int)(ndi->getHeight()/singleRowHeight + 0.5);
            int spanned_j = (int)(ndj->getHeight()/singleRowHeight + 0.5);
            ...
            if( ndi != ndj )
            {
                ...
                if( ndi->getWidth() != ndj->getWidth() || ndi->getHeight() != ndj->getHeight() )
                {
                    std::cout << "Cells " << ndi->getId() << " and " << ndj->getId() << ", "
                ...
                if( spanned_i != new_segs.size() )
                {
                    ...
                    std::cout << "Cell " << ndi->getId() << ", Rows spanned is " << spanned_i << ", "
                        << "assigned to position occupied by cell " << ndj->getId() << ", Rows spanned is " << spanned_j << ", "
                        << "found in only " << new_segs.size() << " segments." << std::endl;
                    exit(-1);
                }
    ...
    //std::cout << "Flow-based replacement; Moved " << nMoved << " cells of " << m_neighbours.size() << ", "
...
double DetailedMis::getHpwl( Node* ndi, double xi, double yi )
{
    ...
    for( int pi = ndi->m_firstPin; pi < ndi->m_lastPin; pi++ )
    {
        ...
        for( int pj = edi->m_firstPin; pj < edi->m_lastPin; pj++ )
        {
            ...
            x = (ndj == ndi) ? (xi + pinj->m_offsetX) : (ndj->getX() + pinj->m_offsetX);
            y = (ndj == ndi) ? (yi + pinj->m_offsetY) : (ndj->getY() + pinj->m_offsetY);
