...
void Graph::CalNetEdgePaths(BundledNet& net)
{
  ...
  if (parents_.find(net.terminals.first) == parents_.end())
    CalShortPathParentVertices(net.terminals.first); // calculate parent vertices
  // initialize an empty path
  std::vector<int> path;
  std::vector<std::vector<int> > paths;  // paths in vertex id
  CalShortPaths(paths, path, parents_[net.terminals.first], 
                net.terminals.second); // pathes in vertex id
     ...
     std::vector<Vertex>& vertex_list)
{
  ...
  for (const auto& soft_macro : soft_macros) {
    x_bound_point.insert(std::floor(soft_macro.GetX()));
    y_bound_point.insert(std::floor(soft_macro.GetY()));
    ...
    x_bound_point.insert(std::floor(soft_macro.GetX() + soft_macro.GetWidth()));
    y_bound_point.insert(std::floor(soft_macro.GetY() + soft_macro.GetHeight()));
  }
  ...
  while (it != x_bound_point.end()) {
    ...
    if (it != x_bound_point.end()) {
      midpoint = std::floor((midpoint + *it) / 2.0);
  ...
  int macro_id = 0;
  for (const auto& soft_macro : soft_macros) {
    const float lx = std::floor(soft_macro.GetX());
    const float ly = std::floor(soft_macro.GetY());
    const float ux = std::floor(lx + soft_macro.GetWidth());
    const float uy = std::floor(ly + soft_macro.GetHeight());
    const float cx = std::floor(lx + soft_macro.GetWidth() / 2.0);
    const float cy = std::floor(ly + soft_macro.GetHeight() / 2.0);
