...
TEST_F(Mpl2PusherTest, ConstructedCentralizedMacros)
{
  ...
  // and check that the XDBU, YDBU values remain the same because Pusher
  // detected it as a centralized macros array (no SoftMacro area).
...
{
  // Create a simple "root" cluster with 2 HardMacroCluster children
  // then push this cluster to the boundaries. Check resulting
  // macro positions.
  ...
  Cluster* child_cluster1
      = new Cluster(1, std::string("hard_macro_cluster_1"), logger);
  ...
  Cluster* child_cluster2
      = new Cluster(2, std::string("hard_macro_cluster_2"), logger);
  ...
  child_cluster2->setX(0.4);
  child_cluster2->setY(0.5);
  ...
  // originally at (100, 200)
  EXPECT_TRUE(macro1->getXDBU() == 0);
  EXPECT_TRUE(macro1->getYDBU() == 0);

  // originally at (400, 500)
  EXPECT_TRUE(macro2->getXDBU() == 0);
  EXPECT_TRUE(macro2->getYDBU() == 500);
