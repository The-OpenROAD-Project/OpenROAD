...
void MD5::decode(unsigned *output, const unsigned char *input, unsigned len) {
        ...
        for (i = 0, j = 0; j < len; i++, j += 4) output[i] = ((unsigned)input[j]) | (((unsigned)input[j + 1]) << 8) | (((unsigned)input[j + 2]) << 16) | (((unsigned)input[j + 3]) << 24);
}
...
void MD5::_update(const unsigned char *input, unsigned inputLen) {
        ...
        if ((_count[0] += ((unsigned)inputLen << 3)) < ((unsigned)inputLen << 3)) _count[1]++;
 ...
 */
        // MD5_memset ((POINTER)context, 0, sizeof (*context)); //we'll do in
        // dtor
        _isFinalized = true;
}
