           ...
           idx_t npes, idx_t *options, idx_t *perm, idx_t *iperm, idx_t *sizes) 
{
  ...
  ctrl = SetupCtrl(METIS_OP_OMETIS, options, 1, 3, NULL, NULL,0, nvtxs);
           ...
           idx_t *vwgt, idx_t *options, idx_t *r_sepsize, idx_t *part) 
{
  ...
  if ((ctrl = SetupCtrl(METIS_OP_OMETIS, options, 1, 3, NULL, NULL,0,*nvtxs)) == NULL)
           ...
           idx_t *where, idx_t *hmarker, real_t ubfactor)
{
  ...
  ctrl = SetupCtrl(METIS_OP_OMETIS, NULL, 1, 3, NULL, NULL,0,nvtxs);
  if (!ctrl) return METIS_ERROR_INPUT;
