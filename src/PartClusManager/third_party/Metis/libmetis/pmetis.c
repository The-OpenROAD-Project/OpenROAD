          ...
          idx_t *objval, idx_t *part)
{
  ...
  ctrl = SetupCtrl(METIS_OP_PMETIS, options, *ncon, *nparts, tpwgts, ubvec,0, *nvtxs);
  if (!ctrl) {
    gk_siguntrap();
...
int METIS_CoarsenGraph(idx_t *nvtxs, idx_t *ncon, idx_t *xadj, 
          idx_t *adjncy, idx_t *vwgt, idx_t *vsize, idx_t *adjwgt, 
          idx_t *nparts, real_t *tpwgts, real_t *ubvec, idx_t *options, 
          idx_t *objval, idx_t *part, idx_t * levels)
{
  int sigrval=0, renumber=0;
  graph_t *graph;
  graph_t *cgraph;
  ...
  /* set up the run parameters */
  ctrl = SetupCtrl(METIS_OP_PMETIS, options, *ncon, *nparts, tpwgts, ubvec,1, *nvtxs);
  if (!ctrl) {
    gk_siguntrap();
    return METIS_ERROR_INPUT;
  }
  ...
  /* allocate workspace memory */
  AllocateWorkSpace(ctrl, graph);
  if (ctrl->finalMap == NULL)
	ctrl->finalMap = malloc((graph->nvtxs+1)*sizeof(idx_t));
  int k;

  for (k=0; k < graph->nvtxs; k++)
	ctrl->finalMap[k] = -1;

  cgraph = CoarsenGraphNlevels(ctrl, graph, *levels);
  for (k=0; k < graph->nvtxs; k++)
	part[k] = ctrl->finalMap[k];
