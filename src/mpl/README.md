...
| `-write_macro_placement` | Generates a file with the design's macro placement in the format of calls for the `place_macro` command. This file does not include the locations that the macro placer generates for the standard cells. |
| `-keep_clustering_data` | Save the hierarchy generated by MPL's clustering engine in the form of dbGroups inside ODB. |
| `-use_full_halo` | Remove pin awareness from halo assignment. |
...
Runs the checks `rtl_macro_placer` makes before it clusters, and stops: no
clustering, no annealing, no report directory, and the design is left as it
was. A flow can run it right after the floorplan is initialised, long before
macro placement, so an infeasible macro placement fails in seconds with the
same messages `rtl_macro_placer` would give: the global fence inside the core
(MPL-0068), every macro with its halo fits in the core (MPL-0006), the
movable cells fit in the macro placement area (MPL-0065), no
fixed standard cell inside the macro placement area (MPL-0050), and the
macros with their halos plus the standard cells fit in the floorplan
(MPL-0016). The halos are the ones `set_macro_base_halo` and `set_macro_halo`
set.

Returns 1 when there are macros to place and the checks pass (MPL-0079), and
0 when every macro is already fixed (MPL-0017); a failed check is an error.
...
| Switch Name | Description |
| ----- | ----- |
| `-fence_lx`, `-fence_ly`, `-fence_ux`, `-fence_uy` | The global fence, as for `rtl_macro_placer`. The default is the core. |
| `-use_full_halo` | Remove pin awareness from halo assignment, as for `rtl_macro_placer`. |
