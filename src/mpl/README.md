...
| `-keep_clustering_data` | Save the hierarchy generated by MPL's clustering engine in the form of dbGroups inside ODB. The clusters can then be browsed and colored in the [web viewer](../web/README.md)'s Hierarchy panel, under its Clusters view, in the Qt GUI with `select -type Group -name <pattern> -highlight 0`, or plotted headlessly with `save_image -web -display_option {cluster_view true}`. An instance belongs to exactly one dbGroup, so instances already owned by another group — a UPF power domain, a region the placer honors — are left in it and reported once (MPL-0078) instead of being taken; they simply do not appear in the clustering data. |
