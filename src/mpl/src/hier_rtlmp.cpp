...
void HierRTLMP::calculateMacroTilings(Cluster* cluster)
{
  ...
  TilingList tilings = getTightPackingTilings(cluster);
  ...
  if (!cluster->isArrayOfInterconnectedMacros()) {
    ...
    // we only keep the minimum area tiling since all the macros has the same
    // size later this can be relaxed.  But this may cause problems because the
    ...
    for (auto& tiling : valid_tilings) {
      if (tiling.area() <= min_tiling_area
          && std::ranges::find(tilings, tiling) == tilings.end()) {
        tilings.push_back(tiling);
      }
    }
  }

  std::ranges::sort(tilings, isAreaSmaller);
  cluster->setTilings(tilings);
