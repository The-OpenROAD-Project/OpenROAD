# test that repair channel straps are unaffected by a flipped instance
#
# Repair channel straps derive their own position from the channel geometry
# rather than from a user offset, so flipping the macros must not move them.
# Same setup as macros_narrow_channel with the two macros flipped on opposite
# axes.
source "helpers.tcl"
...
add_pdn_stripe -layer metal4 -width 0.48 -spacing 4.0 -pitch 65.0 -offset 2.0
...
set def_file [make_result_file macros_flipped_narrow_channel.def]
write_def $def_file
diff_files macros_flipped_narrow_channel.defok $def_file
