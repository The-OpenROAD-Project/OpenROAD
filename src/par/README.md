...
There are 3 main steps, mainly 1) vertex embedding generation, 2) tree construction and partitioning, 3) cut-overlay clustering and ILP-based Partitioning and 4) solution refinement. We will go through each step in turn.

1. Vertex Embedding Generation
- An initial vertex embedding with solution $S_i$ is generated as a hint.
- For bipartitioning ($K=2$), we solve a generalized eigenvalue problem to obtain the first $m$ nontrivial eigenvectors
$X \in \mathcal{R}^{|V|\times m}$
- For multi-way partitioning ($K>2$), we decompose the $K$-way solution into $K$ bipartitioning solutions. Then these will be solved together in a generalized eigenvalue problem, from which we obtain the first $m$ nontrivial eigenvector per solution. From this, we obtain $K \times m $ eigenvectors. Dimension reduction is then performed using Linear Discriminant Analysis (LDA).
...
- Algorithm picks the top $\sigma$ solutions from Step 2, and overlays these solution to generate a clustered hypergraph.
...
| `-thr_coarsen_vertices` | number of vertices of coarsest hypergraph (default 10) |
