...
class PartitionMgr
{
 public:
  ...
  void getFromODB(std::unordered_map<std::string, std::pair<int, bool>>& onlyUseMasters,
                  std::string& top_name,
                  int& numInsts,
                  int& numPIs,
                  int& numPOs,
                  int& numSeq);
  void getFromSTA(int& Dmax,
                  int& MDmax);
  void BuildTimingPath(int& Dmax,
                       int& MDmax);
  void getFromPAR(float& Rratio,
                  float& p,
                  float& q);
  void getRents(float& Rratio,
                float& p,
                float& q);
