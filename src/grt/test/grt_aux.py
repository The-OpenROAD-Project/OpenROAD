...
def check_routing_layer_by_index(design, layer_idx):
    ...
    if layer_idx > max_routing_layer:
        utl.error(utl.GRT, 505, f"Layer {tech_layer.getConstName()} is greater "
                                f"than the max routing layer "
                                f"({max_tech_layer.getConstName()}).")
        
    if layer_idx < 0:
        utl.error(utl.GRT, 506, f"Layer {tech_layer.getConstName()} is less "
...
def define_layer_range(design, layers):
    ...
    for layer in range(lmin, lmax+1):
        ...
        if (not layer_has_tracks(design, db_layer, orient=Orient.HORZ) and
            not layer_has_tracks(design, db_layer, orient=Orient.VERT)):
...
def define_clock_layer_range(design, layers):
    ...
    if mincl < maxcl:
        ...
        utl.error(utl.GRT, 510, "In setting clock layers, min routing layer is greater than max routing layer.")
