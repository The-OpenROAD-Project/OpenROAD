             ...
             const Rsyn::PhysicalDesign& physicalDesign)
{
  ...
  for (const Rsyn::PhysicalObstacle& phyObs : phyLibCell.allObstacles()) {
    if (phyObs.getLayer().getType() != Rsyn::PhysicalLayerType::ROUTING) {
      continue;
    }
    ...
    for (auto bounds : phyObs.allBounds()) {
      bounds.translate(origin);
      bounds = transform.apply(bounds);
      obstacles.emplace_back(layerIndex,
                             bounds.getLower().x,
                             bounds.getLower().y,
                             bounds.getUpper().x,
                             bounds.getUpper().y);
...
int Macro::getPinIndex(std::string pinName)
{
  ...
  if (p == pinIndices.end()) {
    log() << "Error: cannot find pin " << pinName << " in macro " << name
          << std::endl;
    return -1;
  } else {
    return p->second;
...
Instance::Instance(int idx,
                   std::string _name,
                   const Rsyn::PhysicalPort& phyPort)
    : index(idx), name(_name), type(PORT)
{
  auto position = phyPort.getPosition();
  auto bounds = phyPort.getBounds();
  Rsyn::PhysicalTransform transform(Bounds(position, position),
                                    phyPort.getOrientation());
