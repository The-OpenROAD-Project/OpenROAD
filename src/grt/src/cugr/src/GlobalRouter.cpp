...
void GlobalRouter::route()
{
  ...
  log() << netIndices.size() << " / " << nets.size() << " nets have overflows."
        << std::endl;
  logeol();
  ...
  if (netIndices.size() > 0) {
    ...
    log() << netIndices.size() << " / " << nets.size()
          << " nets have overflows." << std::endl;
    logeol();
  }
  ...
  for (auto& gpts : net.getPinAccessPoints()) {
    for (auto& gpt : gpts) {
      if (gpt.layerIdx < parameters.min_routing_layer) {
        int padding = 0;
        if (getSpareResource({parameters.min_routing_layer, gpt.x, gpt.y})
            < parameters.pin_patch_threshold) {
          padding = parameters.pin_patch_padding;
        }
        for (int layerIdx = gpt.layerIdx;
             layerIdx <= parameters.min_routing_layer + 1;
             layerIdx++) {
          guides.emplace_back(
              layerIdx,
              utils::BoxT<int>(
        ...
        for (int c = l; c <= h; c++) {
          ...
          if (getSpareResource(point) < wire_patch_threshold) {
            for (int layerIndex = node->layerIdx - 1;
                 ...
                 layerIndex += 2) {
              if (layerIndex < parameters.min_routing_layer
...
void GlobalRouter::printStatistics() const
{
  ...
  for (int layerIndex = parameters.min_routing_layer;
    ...
    for (int x = 0; x < gridGraph.getSize(0) - 1 + direction; x++) {
      for (int y = 0; y < gridGraph.getSize(1) - direction; y++) {
        ...
        CapacityT capacity
            = max(gridGraph.getEdge(layerIndex, x, y).capacity, 0.0);
  ...
  log() << "wire length (metric):  " << wireLength / gridGraph.getM2Pitch()
        << std::endl;
...
void GlobalRouter::write(std::string guide_file)
{
  ...
  log() << "total area of pin access patches: " << areaOfPinPatches
        << std::endl;
  log() << "total area of wire segment patches: " << areaOfWirePatches
        << std::endl;
  log() << std::endl;
