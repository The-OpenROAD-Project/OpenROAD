      ...
      for (auto& gpt : gpts) {
        if (gpt.getLayerIdx() < constants_.min_routing_layer) {
          ...
          for (int layerIdx = gpt.getLayerIdx();
               ...
               layerIdx++) {
            GSegment segment(
                grid_graph_->getGridline(0, std::max(gpt.x - padding, 0))
                    + half_gcell_size,
                grid_graph_->getGridline(1, std::max(gpt.y - padding, 0))
                    + half_gcell_size,
                layerIdx + 1,
                grid_graph_->getGridline(
                    0,
            ...
            if (getSpareResource(point) < wire_patch_threshold) {
              for (int layerIndex = node->getLayerIdx() - 1;
                ...
                if (getSpareResource({layerIndex, point.x, point.y}) >= 1.0) {
                  GSegment segment(grid_graph_->getGridline(0, point.x)
                                       + half_gcell_size,
                                   grid_graph_->getGridline(1, point.y)
                                       + half_gcell_size,
                                   layerIndex + 1,
                                   grid_graph_->getGridline(0, point.x)
                                       + half_gcell_size,
                                   grid_graph_->getGridline(1, point.y)
                                       + half_gcell_size,
                                   layerIndex + 1,
