...
NetRouteMap CUGR::getPlanarRoutes() 
{
  NetRouteMap routes;
  sttrees_.clear();
  sttrees_.resize(gr_nets_.size());
  for (const int& netIndex : net_indices_) {
    auto gr_net = gr_nets_[netIndex].get();
    odb::dbNet* db_net = gr_net->getDbNet();
    GRoute& route = routes[db_net];
    convertGRTreeToStTree(gr_net, sttrees_[netIndex]);
...
void CUGR::get3DRoute(odb::dbNet* db_net, GRoute& route)
{
  int netID;
  bool exists;
  ...
  for (int edgeID = 0; edgeID < num_edges; edgeID++) {
    const CugrTreeEdge* treeedge = &(treeedges[edgeID]);
    if (treeedge->route.routelen > 0) {
      int routeLen = treeedge->route.routelen;
      const std::vector<CugrGPoint3D>& grids = treeedge->route.grids;
      const int num_terminals = sttrees_[netID].num_terminals;
      const auto& treenodes = sttrees_[netID].nodes;
      int node1_alias = treeedge->n1a;
      int node2_alias = treeedge->n2a;

      std::vector<CugrGPoint3D> filled_grids;
      ...
      int lastX = grid_graph_->getGridline(0, filled_grids[0].x);
      int lastY = grid_graph_->getGridline(1, filled_grids[0].y);
      int lastL = filled_grids[0].layer;

      for (int i = 1; i < filled_grids.size(); i++) {
        const int xreal = grid_graph_->getGridline(0, filled_grids[i].x);
        const int yreal = grid_graph_->getGridline(1, filled_grids[i].y);
...
void CUGR::getNetId(odb::dbNet* db_net, int& net_id, bool& exists)
{
  if (db_net_map_.find(db_net) == db_net_map_.end()) {
    exists = false;
    return;
  }
  net_id = db_net_map_[db_net]->getIndex();
  exists = true;
}
...
void CUGR::convertGRTreeToStTree(const GRNet* net, CugrStTree& st_tree)
{
  // Clear tree
  st_tree.nodes.clear();
  st_tree.edges.clear();
  st_tree.node_to_pin_idx.clear();

  st_tree.num_terminals = net->getNumPins();

  // Create mapping from coordinates to node index
  std::map<GRPoint, int> point_to_node_idx;
  const auto& pin_aps = net->getPinAccessPoints();

  // Populate terminals
  for (int i = 0; i < st_tree.num_terminals; ++i) {
    st_tree.nodes.emplace_back();
  ...
  std::function<int(const std::shared_ptr<GRTreeNode>&)> visit_node
      = [&](const std::shared_ptr<GRTreeNode>& gr_node) -> int {
    int current_idx = -1;

    // Match with pins logic
    for (int i = 0; i < net->getNumPins(); ++i) {
      for (const auto& ap : pin_aps[i]) {
        if (ap.x() == gr_node->x() && ap.y() == gr_node->y()) {
          // Match found (ignoring layer for x,y match, assuming single matching
          // pin loc)
          current_idx = i;
          st_tree.nodes[i].x = gr_node->x();
          st_tree.nodes[i].y = gr_node->y();
          st_tree.nodes[i].botL = gr_node->getLayerIdx();
          st_tree.nodes[i].topL = gr_node->getLayerIdx();
          break;
        }
      }
      if (current_idx != -1) {
    ...
    if (current_idx == -1) {
      current_idx = st_tree.nodes.size();
      st_tree.nodes.emplace_back();
      st_tree.nodes.back().x = gr_node->x();
      st_tree.nodes.back().y = gr_node->y();
      st_tree.nodes.back().botL = gr_node->getLayerIdx();
      st_tree.nodes.back().topL = gr_node->getLayerIdx();
    }

    for (const auto& child : gr_node->getChildren()) {
      int child_idx = visit_node(child);

      CugrTreeEdge edge;
      edge.n1 = current_idx;
      edge.n2 = child_idx;
      edge.n1a = current_idx;
      edge.n2a = child_idx;
      edge.len = std::abs(gr_node->x() - child->x())
                 + std::abs(gr_node->y() - child->y());

      int x1 = gr_node->x();
      int y1 = gr_node->y();
      int l1 = gr_node->getLayerIdx();
      int x2 = child->x();
      int y2 = child->y();
      int l2 = child->getLayerIdx();

      // Populate route
      edge.route.grids.push_back({(int16_t) x1, (int16_t) y1, (int16_t) l1});
      if (l1 == l2) {
        if (x1 != x2) {
          edge.route.grids.push_back(
              {(int16_t) x2, (int16_t) y1, (int16_t) l1});
        }
        if (y1 != y2) {
          edge.route.grids.push_back(
              {(int16_t) x2, (int16_t) y2, (int16_t) l1});
        }
      } else {
        int l_start = std::min(l1, l2);
        int l_end = std::max(l1, l2);
        for (int l = l_start; l <= l_end; ++l) {
          if (l == l1) {
            continue;
          }
          edge.route.grids.push_back({(int16_t) x1, (int16_t) y1, (int16_t) l});
        }
      }
      edge.route.routelen = edge.route.grids.size() - 1;

      st_tree.edges.push_back(edge);
      int edge_idx = st_tree.edges.size() - 1;

      // Populate hID/lID for connectivity checks
      auto addEdgeToNode = [&](int nodeIdx, int eIdx) {
        if (st_tree.nodes[nodeIdx].hID == -1) {
          st_tree.nodes[nodeIdx].hID = eIdx;
        } else if (st_tree.nodes[nodeIdx].lID == -1) {
          st_tree.nodes[nodeIdx].lID = eIdx;
        }
      };
      addEdgeToNode(current_idx, edge_idx);
      addEdgeToNode(child_idx, edge_idx);
    }
    return current_idx;
  };
