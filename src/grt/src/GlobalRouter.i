...
% inline %{
  ...
  void set_congestion_iterations(int iterations)
  {
    getGlobalRouter()->setCongestionIterations(iterations);
  }

  void set_congestion_report_iter_step(int congestion_report_iter_step)
  {
  ...
  void set_grid_origin(int x, int y)
  {
    getGlobalRouter()->setGridOrigin(x, y);
  }

  void set_allow_congestion(bool allowCongestion)
  {
    getGlobalRouter()->setAllowCongestion(allowCongestion);
  }

  void set_resistance_aware(bool resistance_aware)
  {
    getGlobalRouter()->setResistanceAware(resistance_aware);
  }

  void set_critical_nets_percentage(float criticalNetsPercentage)
  {
    getGlobalRouter()->setCriticalNetsPercentage(criticalNetsPercentage);
  }

  void set_macro_extension(int macroExtension)
  {
  ...
  {
    // Save guides by default when ending incremental routing from Tcl
    // interface.
  ...
  std::vector<int> route_layer_lengths(odb::dbNet* db_net)
  {
    return getGlobalRouter()->routeLayerLengths(db_net);
  }

  int repair_antennas(odb::dbMTerm* diode_mterm,
                      int iterations,
                      float ratio_margin,
                      bool jumper_only,
                      bool diode_only)
  {
    ...
    return getGlobalRouter()->repairAntennas(diode_mterm,
                                             iterations,
                                             ratio_margin,
                                             jumper_only,
                                             diode_only,
  ...
  void add_net_to_route(odb::dbNet* net)
  {
    getGlobalRouter()->addNetToRoute(net);
  }

  void highlight_net_route(odb::dbNet* net, bool show_pin_locations)
  {
    if (!gui::Gui::enabled()) {
      return;
    }
    ...
    if (router->getRenderer() == nullptr) {
      router->setRenderer(
          std::make_unique<GrouteRenderer>(router, router->db()->getTech()));
    }

    router->getRenderer()->highlightRoute(net, show_pin_locations);
  }

  void read_guides(const char* fileName)
  {
  ...
  void report_layer_wire_lengths(bool global_route, bool detailed_route)
  {
