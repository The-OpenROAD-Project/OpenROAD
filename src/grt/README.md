...
The `-st`/`-rst`/`-tree2D`/`-tree3D`/`-edges3D` flags apply to the default
FastRoute engine; the `-cugr_*` flags apply to the CUGR engine
(`global_route -use_cugr`). Each selected stage redraws the net and pauses,
so you step through the topology one stage at a time.

Each enabled `-cugr_*` stage can also trace the debug net's slack, so the
timing effect of each stage is visible without the GUI (only the topology
drawing needs a GUI). The trace is a debug print, enabled with:

```tcl
set_debug_level GRT cugr_slack 1
```

Without it nothing is printed and no timing work is done. Each line reports the
stage's result and the input CUGR used to produce it:

- `slack` -- the stage's **result**. CUGR defers each stage's parasitics to the
  next stage's refresh, so parasitics are brought up to date first, exactly as
  that refresh would: every net the stage rerouted, not just the debug net,
  since the debug net's slack runs through paths in those nets too. The dirty
  net list is left intact so the real refresh still runs and recomputes the
  same values, and routing output is unchanged. This re-extraction is
  duplicated work, which is why it is gated on the debug level rather than on
  the `-cugr_*` flags.
- `cugr entered stage with` -- the slack cached on the net, which is what
  CUGR's net ordering, critical-net selection and resistance-aware scoring
  actually read for this stage. `delta` is the difference, i.e. what the stage
  did to the net's slack.

Each line's `slack` therefore equals the next line's `entered stage with`, so
the trace reads as a continuous history.

A net that fell outside `-critical_nets_percentage` reports
`cugr entered stage demoted, sorts last`: CUGR overwrites a non-critical net's
slack with a sentinel so it sorts last, and therefore holds no timing value for
it. Slack is reported as `unavailable` when no liberty library is loaded, and as
`no timing path` when STA finds none.

A `-cugr_*` flag pauses only when its stage actually reroutes something. CUGR
skips a stage when the previous one left no congested nets to work on, and it
skips the resistance-aware stage entirely unless `-resistance_aware` is set
with a non-zero `-critical_nets_percentage`. A flag whose stage is skipped is
silently inactive rather than redrawing the previous stage's topology.
...
| `-cugr_pattern` | CUGR: show the topology after pattern routing (stage 1). |
| `-cugr_res_aware` | CUGR: show the topology after the resistance-aware re-route of the critical nets (stage 2). |
| `-cugr_detours` | CUGR: show the topology after pattern routing with detours (stage 3). |
| `-cugr_maze` | CUGR: show the topology after maze routing on the sparsified graph (stage 4). |
| `-cugr_rrr` | CUGR: show the topology after each iterative rip-up and re-route iteration (stage 5). |
