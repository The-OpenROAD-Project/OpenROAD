...
////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
DetailedInterleave::DetailedInterleave( Architecture* arch, Network* network, RoutingParams* rt ):
    m_arch( arch ),
    m_network( network ),
    m_rt( rt ),
...
//////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////
void DetailedInterleave::run( DetailedMgr* mgrPtr, std::vector<std::string>& args )
{
    m_mgrPtr = mgrPtr;
    ...
    m_mgrPtr->resortSegments();
    std::cout << boost::format( "End of passes for interleaving; hpwl is %.6e, total imp is %.2lf%%\n" )
...
void DetailedInterleave::dp( void )
{
    ...
    for( size_t s = 0; s < m_mgrPtr->getNumSegments(); s++ )
    {
        ...
        //std::cout << "Interleaving segment " << segId << ", Cells is " << nodes.size() << std::endl;
        ...
        while( j < n )
        {
            ...
            //std::cout << " => Span on single height cells " << n << ", [" << jstrt << " -> " << jstop << "]" << std::endl;
            ...
            for( int i = jstrt; i <= jstop; i+=m_windowStep )
            {
                ...
                //std::cout << "  => Window, [" << istrt << " -> " << istop << "]" << std::endl;
                ...
                //std::cout << "Initial cost is " << initCost << ", current cost is " << nextCost << std::endl;
                ...
                std::sort( nodes.begin() + jstrt, nodes.begin() + (jstop+1), compareNodesX() ); 
            }
        }
    }
    return;
    ...
    int jstrt, int jstop )
{
    ...
    for( int j = jstrt; j <= jstop; j++ )
    {
        ...
        for( int pj = nd->m_firstPin; pj < nd->m_lastPin; pj++ )
        {
            Pin* pin = m_network->m_nodePins[pj];
            Edge* ed = &(m_network->m_edges[pin->m_edgeId]);
            int npins = ed->m_lastPin - ed->m_firstPin;
    ...
    // Setup up interval and cell widths, including padding and extra spacing.
    {
        ...
        double rightEdge = std::min( ndr->getX() + 0.5 * ndr->getWidth() + rightPadding, rightLimit );
        m_arch->getCellPadding( ndl, leftPadding, dummyPadding );
        double leftEdge = std::max( ndl->getX() - 0.5 * ndl->getWidth() - leftPadding, leftLimit );
        ...
        if( rightEdge - leftEdge >= totalWidth + totalPadding )
        {
            ...
            double amtPerCell = ((rightEdge-leftEdge)-(totalWidth+totalPadding)) / (double)sm->m_nNodes;
        ...
        else if( rightEdge - leftEdge >= totalWidth )
        {
            ...
            double amtPerCell = ((rightEdge-leftEdge)-(totalWidth)) / (double)sm->m_nNodes;
            int sitesPerCell = std::max( 1, (int)std::floor( amtPerCell / siteWidth ) );
    ...
    for( size_t e = 0; e < m_edgeIds.size(); e++ )
    {
        ...
        EdgeInterval tmp;               
        for( int pi = ed->m_firstPin; pi < ed->m_lastPin; pi++ )
        {
            Pin* pin = m_network->m_edgePins[pi];
            Node* nd = &(m_network->m_nodes[pin->m_nodeId]);
            double x = nd->getX() + pin->m_offsetX;
            x = std::min( std::max( x, sm->m_xmin ), sm->m_xmax ); 
            if( m_nodeMask[nd->getId()] == m_traversal )
            {
                int nid = m_nodeMap[nd->getId()];
                sm->m_adjEdges[ nid ].push_back( EdgeAndOffset( eid, pin->m_offsetX ) );
            }
            else
            {
...
void DetailedInterleave::dp( std::vector<Node*>& nodes, double minX, double maxX )
{
    ...
    if( nodes.size() > 200 )
    {
        std::cout << "Interleave, direct call skipped; nodes is " << nodes.size() << std::endl;
        return;
    }
 ...
 */
    ...
    for( size_t e = 0; e < m_edgeIds.size(); e++ )
    {
        ...
        for( int pi = ed->m_firstPin; pi < ed->m_lastPin; pi++ )
        {
            Pin* pin = m_network->m_edgePins[pi];
            Node* nd = &(m_network->m_nodes[pin->m_nodeId]);
            double x = std::min( std::max( nd->getX() + pin->m_offsetX, minX ), maxX );
            if( m_nodeMask[nd->getId()] == m_traversal )
            {
                int nid = m_nodeMap[nd->getId()];
                sm.m_adjEdges[ nid ].push_back( EdgeAndOffset( eid, pin->m_offsetX ) );
