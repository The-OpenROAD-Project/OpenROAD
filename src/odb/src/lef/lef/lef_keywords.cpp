...
void lefReloadBuffer()
{
  ...
  if (lefData->first_buffer) {
    ...
    lefData->encrypted
        = encIsEncrypted((unsigned char*) lefData->current_buffer);
  }
...
{
  if ((lefData->next <= lefData->current_buffer)
      || (lefData->input_level > 0)) {
...
void lefAddBooleanDefine(const char* token, int val)
{
  lefData->defineb_set[strip_case(token)] = val;
}

void lefAddNumDefine(const char* token, double val)
{
...
inline static void print_nlines(int lineNum)
{
...
static int GetToken(char** buffer, int* bufferSize)
{
  ...
  if (ch == '"') {
    do {
      ...
      */
      /* 8/7/2008 - CCR 586175
      ** Some files may not end with \n or \0 or EOF as the lefData->last
      *character
  ...
  } else if (lefSettings->ShiftCase) {  // we are case insensitive, use a
                                        // different loop
    for (;; ch = lefGetc()) {
      /* 5/6/2008 - CCR 556818
      ** Check if the ch is a valid ascii character 0 =< ch < 128
 ...
 * If the global "lefData->lefDumbMode" is > 0, it reads the lefData->next token
 * in "dumb mode". In this case, it does not do keyword lookup, or attempt to
 * read a token as a number; if the token is not punctuation, it's a T_STRING.
 * Each token read decrements lefData->lefDumbMode, so you can instruct the the
 * lexer to read the lefData->next N tokens in dumb mode by setting
 * "lefData->lefDumbMode" to that value.
 *
 * Newlines are in general silently ignored.  If the global lefData->lefNlToken
 * is true, however, they are returned as the token K_NL.
 */
...
int lefsublex()
{
  ...
  if (lefData->lefInvalidChar) {
    ...
    sprintf(outStr,
            "Invalid characters found in \'%s\'.\nThese characters might have "
            "created by character types other than English.",
  ...
  if (lefData->lefDumbMode >= 0) {
    ...
    if (lefData->lefNewIsKeyword
        && strcmp(lefData->current_token, "NEW") == 0) {
...
{
  char* str;
  const char* curToken = isgraph(lefData->current_token[0])
                             ? lefData->current_token
                             : "<unprintable>";
  const char* pvToken
      = isgraph(lefData->pv_token[0]) ? lefData->pv_token : "<unprintable>";
  ...
  if (strcmp(s, "parse error") == 0) {
    if ((len > 1) && (lefData->current_token[len] == ';')) {
      str = (char*) lefMalloc(len + strlen(s) + strlen(lefData->lefrFileName)
                              + 350);
      sprintf(str,
              "ERROR (LEFPARS-%d): %s, see file %s at line %d\nLast token was "
              "<%s>, space is missing before <;>\n",
              msgNum,
              s,
      ...
      str = (char*) lefMalloc(pvLen + strlen(s) + strlen(lefData->lefrFileName)
                              + 350);
      sprintf(str,
              "ERROR (LEFPARS-%d): %s, see file %s at line %d\nLast token was "
              "<%s>, space is missing before <;>\n",
      ...
      sprintf(str,
              "ERROR (LEFPARS-%d): %s, see file %s at line %d\nLast token was "
              "<%s\">, space is missing between the closing \" of the string "
              "and ;.\n",
              1010,
              s,
              lefData->lefrFileName,
      ...
      sprintf(str,
              "ERROR (LEFPARS-%d): Lef parser has encountered an error in file "
              "%s at line %d, on token %s.\nProblem can be syntax error on the "
              "lef file or an invalid parameter name.\nDouble check the syntax "
              "on the lef file with the LEFDEF Reference Manual.\n",
              msgNum,
              lefData->lefrFileName,
              lefData->lef_nlines,
              curToken);
...
void lefInfo(int msgNum, const char* s)
{
  ...
  if (disableStatus == 1) {
    char msgStr[60];
    sprintf(msgStr,
            "Message (LEFPARS-%d) has been suppressed from output.",
            msgNum);
  ...
  if (lefSettings->WarningLogFunction) {
    char* str = (char*) lefMalloc(strlen(lefData->current_token) + strlen(s)
    ...
    if (!lefData->hasOpenedLogFile) {
      if ((lefData->lefrLog = fopen("lefRWarning.log", "w")) == nullptr) {
        ...
        fprintf(
            lefData->lefrLog, "Info from file: %s\n\n", lefData->lefrFileName);
        fprintf(lefData->lefrLog,
                "INFO (LEFPARS-%d): %s See file %s at line %d\n",
                msgNum,
                s,
...
void lefWarning(int msgNum, const char* s)
{
  ...
  if (lefSettings->WarningLogFunction) {
    char* str = (char*) lefMalloc(strlen(lefData->current_token) + strlen(s)
                                  + strlen(lefData->lefrFileName) + 350);
    sprintf(str,
            "WARNING (LEFPARS-%d): %s See file %s at line %d.\n",
            msgNum,
            s,
            lefData->lefrFileName,
            lefData->lef_nlines);
