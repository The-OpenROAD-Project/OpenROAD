              ...
              while ((path = (int) p->next()) != DEFIPATH_DONE) {
                ...
                switch (path) {
                  ...
                  case DEFIPATH_VIA:
                    ...
                    break;
                  case DEFIPATH_VIAROTATION:
                    fprintf(fout, "%s ", p->getViaRotationStr());
                    break;
                  case DEFIPATH_WIDTH:
                    fprintf(fout, "%d ", p->getWidth());
                    break;
  ...
  if (net->numProps()) {
    for (i = 0; i < net->numProps(); i++) {
      ...
      switch (net->propType(i)) {
        case 'R':
          fprintf(fout, "%g REAL ", net->propNumber(i));
          break;
        case 'I':
          fprintf(fout, "%g INTEGER ", net->propNumber(i));
          break;
        case 'S':
          fprintf(fout, "%s STRING ", net->propValue(i));
          break;
        case 'Q':
          fprintf(fout, "%s QUOTESTRING ", net->propValue(i));
          break;
        case 'N':
          fprintf(fout, "%g NUMBER ", net->propNumber(i));
          break;
      }
      fprintf(fout, "\n");
    }
  ...
  if (net->numProps()) {
    for (i = 0; i < net->numProps(); i++) {
      if (net->propIsString(i))
        fprintf(
            fout, "  + PROPERTY %s %s ", net->propName(i), net->propValue(i));
      if (net->propIsNumber(i))
        fprintf(
            fout, "  + PROPERTY %s %g ", net->propName(i), net->propNumber(i));
      switch (net->propType(i)) {
        case 'R':
          fprintf(fout, "REAL ");
          break;
  ...
  switch (c) {
      ...
      if (testDebugPrint) {
        ...
        if (pin->hasLayer()) {
          for (i = 0; i < pin->numLayer(); i++) {
            ...
            if (pin->hasLayerDesignRuleWidth(i))
              fprintf(
                  fout, "DESIGNRULEWIDTH %d ", pin->layerDesignRuleWidth(i));
        ...
        if (pin->hasAPinPartialMetalSideArea()) {
          for (i = 0; i < pin->numAPinPartialMetalSideArea(); i++) {
            fprintf(fout,
                    "ANTENNAPINPARTIALMETALSIDEAREA %d",
        ...
        for (j = 0; j < pin->numAntennaModel(); j++) {
          ...
          if (aModel->hasAPinMaxAreaCar()) {
            for (i = 0; i < aModel->numAPinMaxAreaCar(); i++) {
              fprintf(
                  fout, "ANTENNAPINMAXAREACAR %d", aModel->APinMaxAreaCar(i));
    ...
    case defrRowCbkType:
      ...
      if (row->numProps() > 0) {
        for (i = 0; i < row->numProps(); i++) {
          fprintf(
              fout, "  + PROPERTY %s %s ", row->propName(i), row->propValue(i));
    ...
    case defrTrackCbkType:
      ...
      if (track->firstTrackMask()) {
        if (track->sameMask()) {
          ...
          fprintf(fout,
                  "TRACKS %s %g DO %g STEP %g MASK %d LAYER ",
                  track->macro(),
                  track->x(),
                  track->xNum(),
                  track->xStep(),
    ...
    case defrPartitionCbkType:
      ...
      if (strcmp(itemT, "CLOCK") == 0) {
        ...
        if (part->hasMin())
          fprintf(
              fout, "MIN %g %g ", part->partitionMin(), part->partitionMax());
        if (part->hasMax())
          fprintf(
              fout, "MAX %g %g ", part->partitionMin(), part->partitionMax());
        fprintf(fout, "PINS ");
        ...
        if (dir == 'F')  // fromiopin
          fprintf(
              fout, "+ FROMIOPIN %s %s ", part->instName(), part->pinName());
      } else if (strcmp(itemT, "COMP") == 0) {
        if (dir == 'T')  // tocomppin
          fprintf(
              fout, "+ TOCOMPPIN %s %s ", part->instName(), part->pinName());
...
int main(int argc, char** argv)
{
  ...
  while (argc--) {
    if (strcmp(*argv, "-d") == 0) {
      ...
      fprintf(
          stderr,
          "\tif <defFileName> is set to 'STDIN' - takes data from stdin.\n");
      fprintf(stderr, "Options:\n");
      fprintf(stderr,
              "\t-i <num_lines> -- sets processing msg interval (default: 50 "
              "lines).\n");
      fprintf(stderr,
              "\t-nc            -- no functional callbacks will be called.\n");
