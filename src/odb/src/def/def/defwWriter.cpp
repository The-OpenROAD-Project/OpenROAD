...
enum State
{
  ...
  DEFW_TRACKS = 17,
  DEFW_GCELL_GRID = 18,
  DEFW_DEFAULTCAP_START = 19,
  DEFW_DEFAULTCAP = 20,
  DEFW_DEFAULTCAP_END = 21,
  DEFW_CANPLACE = 22,
  DEFW_CANNOTOCCUPY = 23,
  DEFW_VIA_START = 24,
  DEFW_VIA = 25,
  DEFW_VIAVIARULE = 26,
  DEFW_VIAONE_END = 27,
  DEFW_VIA_END = 28,
  DEFW_REGION_START = 29,
  DEFW_REGION = 30,
  DEFW_REGION_END = 31,
  DEFW_COMPONENT_MASKSHIFTLAYERS = 32,
  DEFW_COMPONENT_START = 33,
  DEFW_COMPONENT = 34,
  DEFW_COMPONENT_END = 35,
  DEFW_PIN_START = 36,
  DEFW_PIN = 37,
  DEFW_PIN_PORT = 38,
  DEFW_PIN_END = 39,
  DEFW_PINPROP_START = 40,
  DEFW_PINPROP = 41,
  DEFW_PINPROP_END = 42,
  DEFW_BLOCKAGE_START = 43,
  DEFW_BLOCKAGE_LAYER = 44,
  DEFW_BLOCKAGE_PLACE = 45,
  DEFW_BLOCKAGE_RECT = 46,
  DEFW_BLOCKAGE_POLYGON = 47,
  DEFW_BLOCKAGE_END = 48,
  DEFW_SNET_START = 49,
  DEFW_SNET = 50,
  DEFW_SNET_OPTIONS = 51,
  DEFW_SNET_ENDNET = 52,
  DEFW_SNET_END = 53,
  DEFW_PATH_START = 54,
  DEFW_PATH = 55,
  ...
  DEFW_SUBNET = 62,
  DEFW_NOSHIELD = 63,
  DEFW_IOTIMING_START = 64,
  DEFW_IOTIMING = 65,
  DEFW_IOTIMING_END = 66,
  DEFW_SCANCHAIN_START = 67,
  DEFW_SCANCHAIN = 68,
  DEFW_SCAN_FLOATING = 69,
  DEFW_SCAN_ORDERED = 70,
  DEFW_SCANCHAIN_END = 71,
  DEFW_FPC_START = 72,
  DEFW_FPC = 73,
  DEFW_FPC_OPER = 74,
  DEFW_FPC_OPER_SUM = 75,
  DEFW_FPC_END = 76,
  DEFW_GROUP_START = 77,
  DEFW_GROUP = 78,
  DEFW_GROUP_END = 79,
  ...
  DEFW_NDR = 91,
  DEFW_NDR_END = 92,
  DEFW_STYLES_START = 93,
  DEFW_STYLES = 94,
  DEFW_STYLES_END = 95,
  DEFW_BEGINEXT_START = 96,
  DEFW_BEGINEXT = 97,
  DEFW_BEGINEXT_END = 98,
  DEFW_FILL_LAYERMASK = 99,
  DEFW_FILL_VIAMASK = 100,
  DEFW_BLOCKAGE_MASK = 101,
};

static constexpr int DEFW_END = 102;

static constexpr int DEFW_DONE = 999;

static constexpr int MAXSYN = 103;
...
static void printPoints(FILE* file,
                        double x,
                        double y,
                        const char* prefix,
                        const char* suffix)
...
)
{
  ...
  if (defwHasInitCbk == 1) {  // defwInitCbk has already called, issue an error
    fprintf(stderr,
            "ERROR (DEFWRIT-9000): The DEF writer has detected that the "
            "function defwInitCbk has already been called and you are trying "
            "to call defwInit.\nOnly defwInitCbk or defwInit can be called but "
...
int defwInitCbk(FILE* f)
{
  ...
  if (defwHasInit == 1) {  // defwInit has already called, issue an error
    fprintf(stderr,
            "ERROR (DEFWRIT-9001): The DEF writer has detected that the "
            "function defwInit has already been called and you are trying to "
            "call defwInitCbk.\nOnly defwInitCbk or defwInit can be called but "
            "not both.\nUpdate your program and then try again.\n");
    fprintf(stderr, "Writer Exit.\n");
    exit(DEFW_BAD_ORDER);
...
int defwViaRect(const char* layerNames,
                int xl,
                int yl,
                int xh,
                int yh,
                            ...
                            double* optValue)
{
  ...
  for (i = 0; i < numPts; i++) {
    ...
    fprintf(
        defwFile, " ( %.11g %.11g %.11g )", pointx[i], pointy[i], optValue[i]);
  }
  return DEFW_OK;
}
...
int defwNetPathMask(int colorMask)
{
  if (defVersionNum < 5.8) {
    return DEFW_WRONG_VERSION;
  }
...
int defwNetPathRect(int deltaX1, int deltaY1, int deltaX2, int deltaY2)
{
  ...
  fprintf(
      defwFile, " RECT ( %d %d %d %d )", deltaX1, deltaY1, deltaX2, deltaY2);
...
int defwBlockagesLayerMask(int colorMask)
{
  if (defVersionNum < 5.8) {
    return DEFW_WRONG_VERSION;
  }
...
int defwBlockageMask(int colorMask)
{
  if (defVersionNum < 5.8) {
    return DEFW_WRONG_VERSION;
  }
...
int defwFillViaMask(int maskColor)
{
  if (defVersionNum < 5.8) {
    return DEFW_WRONG_VERSION;
  }
...
void defwPrintError(int status)
{
  switch (status) {
    ...
    case DEFW_BAD_ORDER:
      fprintf(
          defwFile, "%s - Incorrect order of data.\n", defwStateStr[defwFunc]);
      break;
    case DEFW_BAD_DATA:
    ...
    case DEFW_ALREADY_DEFINED:
      fprintf(defwFile,
              "%s - Section is allowed to define only once.\n",
              defwStateStr[defwFunc]);
      break;
    case DEFW_WRONG_VERSION:
      fprintf(defwFile,
              "%s - Version number is set before 5.6, but 5.6 API is used.\n",
              defwStateStr[defwFunc]);
      break;
    case DEFW_OBSOLETE:
      fprintf(defwFile,
              "%s - is no longer valid in 5.6.\n",
