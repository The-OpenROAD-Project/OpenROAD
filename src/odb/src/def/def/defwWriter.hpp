...
#define DEFW_TOO_MANY_STMS \
  7  // the number defined at the beginning of the
...
extern int defwRow(const char* rowName,
                   const char* rowType,
                   int x_orig,
                   int y_orig,
                   int orient,
...
extern int defwRowStr(const char* rowName,
                      const char* rowType,
                      int x_orig,
                      int y_orig,
                      const char* orient,
 ...
 * This is a 5.6 syntax.
 * Returns 0 if successful. */
extern int defwViaViaruleOffset(int xBotOffset,
                                int yBotOffset,
                                int xTopOffset,
                                int yTopOffset);
 ...
 * This is a 5.7 syntax.
 * Returns 0 if successful.  */
extern int defwComponentRouteHalo(int haloDist,
                                  const char* minLayer,
                                  const char* maxLayer);
...
extern int defwPin(const char* name,       /* pinName */
                   ...
                   const char* status,     // optional(NULL) - FIXED | PLACED |
                                           // COVER
                   int statusX,
                   int statusY,       /* optional(0) - status point */
                   int orient,        /* optional(-1) - status orient */
                   const char* layer, /* optional(NULL) - layerName */
                   int xl,
                   int yl, /* optional(0) - layer point1 */
                   int xh,
                   int yh); /* optional(0) - layer point2 */
 ...
 * The routine can be called many times.
 * NOTE: Use defwPinLayer to write out layer with SPACING or DESIGNRULEWIDTH */
extern int defwPinPortLocation(const char* status, /* FIXED | PLACED | COVER */
                               int statusX,
                               int statusY, /* status point */
                               const char* orient);
...
extern int defwPinAntennaPinPartialMetalArea(
    int value,
 ...
 * This is a 5.4 syntax.
 * The routine can be called multiple times. */
extern int defwPinAntennaPinPartialMetalSideArea(
    int value,
    const char* layerName); /* optional(NULL) */
 ...
 * This is a 5.4 syntax.
 * The routine can be called multiple times. */
extern int defwPinAntennaPinPartialCutArea(
    int value,
    const char* layerName); /* optional(NULL) */
 ...
 * This is a 5.4 syntax.
 * The routine can be called multiple times. */
extern int defwPinAntennaPinDiffArea(
    int value,
    const char* layerName); /* optional(NULL) */
 ...
 * This is a 5.4 syntax.
 * The routine can be called multiple times. */
extern int defwPinAntennaPinGateArea(
    int value,
    const char* layerName); /* optional(NULL) */
 ...
 * This is a 5.4 syntax.
 * The routine can be called multiple times. */
extern int defwPinAntennaPinMaxAreaCar(
    int value,
    const char* layerName); /* optional(NULL) */
 ...
 * This is a 5.4 syntax.
 * The routine can be called multiple times. */
extern int defwPinAntennaPinMaxSideAreaCar(
    int value,
    const char* layerName); /* optional(NULL) */
 ...
 * This is a 5.4 syntax.
 * The routine can be called multiple times. */
extern int defwPinAntennaPinMaxCutCar(
    int value,
    const char* layerName); /* optional(NULL) */
...
extern int defwSpecialNetConnection(
    const char* inst, /* compNameRegExpr */
...
extern int defwSpecialNetPathShape(
    const char* shapeType);  // RING | STRIPE |
                             // FOLLOWPIN | IOWIRE | COREWIRE | BLOCKWIRE |
                             // FILLWIRE | BLOCKAGEWIRE
...
extern int defwSpecialNetShieldShape(
    const char* shapeType);  // RING | STRIPE |
                             // FOLLOWPIN | IOWIRE | COREWIRE | BLOCKWIRE |
                             // FILLWIRE | BLOCKAGEWIRE
...
/* A 5.4.1 syntax */
extern int defwSpecialNetShieldViaData(int numX,
                                       int numY,
                                       int stepX,
                                       int stepY);
...
extern int defwNetVpin(const char* vpinName,
                       const char* layerName, /* optional(NULL) */
                       int layerXl,
                       int layerYl, /* layer point1 */
                       int layerXh,
                       int layerYh,        /* layer point2 */
 ...
 * This is a 5.4.1 syntax */
extern int defwScanchainFloating(
    const char* name,  /* floatingComp */
    const char* inst1, /* optional(NULL) - IN | OUT */
    const char* pin1,  /* can't be null if inst1 is set */
    const char* inst2, /* optional(NULL) - IN | OUT */
    const char* pin2); /* can't be null if inst2 is set */
 ...
 * the option BITS. */
extern int defwScanchainFloatingBits(
    const char* name,  /* floatingComp */
    const char* inst1, /* optional(NULL) - IN | OUT */
    const char* pin1,  /* can't be null if inst1 is set */
    const char* inst2, /* optional(NULL) - IN | OUT */
...
extern int defwScanchainOrdered(
    const char* name1,
    const char* inst1, /* optional(NULL) - IN | OUT */
    const char* pin1,  /* can't be null if inst1 is set */
    const char* inst2, /* optional(NULL) - IN | OUT */
 ...
 * the option BITS */
extern int defwScanchainOrderedBits(
    const char* name1,
    const char* inst1, /* optional(NULL) - IN | OUT */
    const char* pin1,  /* can't be null if inst1 is set */
    const char* inst2, /* optional(NULL) - IN | OUT */
...
extern int defwConstraintOperandTime(
    const char* timeType,  //  RISEMAX | FALLMAX | RISEMIN | FALLMIN
    int time);
...
extern int defwGroupRegion(int xl,
                           int yl, /* either the x & y or    */
                           int xh,
                           int yh,                  /* regionName only, can't */
 ...
 * This routine is called per entry within a blockage layer, can't be more then
 * one. This is a 5.4 syntax. */
 ...
 * This routine is called per entry within a blockage layer, can't be more then
 * one. This is a 5.7 syntax. */
extern int defwBlockagesLayerExceptpgnet();

/* This routine is for - component
 * This routine called per entry within a blockage layer, can't be more than
 * one. This is a 5.6 syntax. */
...
/* This routine is for - component
 * This routine is called per entry within blockage placement, can't be more
 * then one. This is a 5.4 syntax. */
