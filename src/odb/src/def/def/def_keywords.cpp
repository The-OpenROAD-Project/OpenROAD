...
int defrData::DefGetTokenFromStack(char* s)
{
  ...
  while (input_level >= 0) {
    for (ch = stack[input_level].c_str(); *ch != 0;
         ch++) { /* skip white space */
...
int defrData::sublex(YYSTYPE* pYylval)
{
  ...
  if (defInvalidChar) {
    ...
    sprintf(outMsg,
            ...
            "using the character types other than English.\nCreate characters "
      ...
      if (K_HISTORY == result) { /* history - get up to ';' */
            ...
            if (cc != ' ') {
              if (cc == '\"') { /* found a quote */
                if (!begQuote) {
                  ...
                  defError(6004,
                           "The BEGINEXT tag is empty. Specify a value for the "
                           "tag and try again.");
                  break;
                }
              } else if (!begQuote) { /* anything but a quote */
                defError(6005,
                         "The '\"' is missing within the tag. Specify the '\"' "
        ...
        if (foundTag) {
          ...
          while (true) {
            ...
            if (histTextSize >= 6
                && memcmp(&History_text[histTextSize - 6], "ENDEXT", 6) == 0) {
              if (begQuote) {
                defError(6006,
...
void defrData::defError(int msgNum, const char* s)
{
  ...
  if (strcmp(s, "parse error") == 0) {
    if ((len > 1) && (deftoken[len] == ';')) {
      ...
      sprintf(str,
              "ERROR (DEFPARS-%d): Def parser has encountered an error in file "
              "%s at line %s, on token %s.\nProblem can be syntax error on the "
              "def file or an invalid parameter name.\nDouble check the syntax "
              "on the def file with the LEFDEF Reference Manual.\n",
              msgNum,
              session->FileName,
              lines2str(nlines),
              curToken);
    ...
    str = (char*) malloc(len + strlen(s) + strlen(session->FileName) + 350);
    sprintf(str,
            "ERROR (DEFPARS-%d): %s Error in file %s at line %s, on token "
...
void defrData::defInfo(int msgNum, const char* s)
{
  ...
  if (settings->ContextWarningLogFunction) {
    ...
    sprintf(str,
            "INFO (DEFPARS-%d): %s See file %s at line %s.\n",
            msgNum,
            s,
            session->FileName,
            lines2str(nlines));
...
void defrData::defWarning(int msgNum, const char* s)
{
  ...
  for (i = 0; i < settings->nDDMsgs;
       i++) { /* check if warning has been disable */
    if (settings->disableDMsgs[i] == msgNum) {
  ...
  if (settings->ContextWarningLogFunction) {
    char* str = (char*) malloc(strlen(deftoken) + strlen(s)
                               + strlen(session->FileName) + 350);
    sprintf(str,
            "WARNING (DEFPARS-%d): %s See file %s at line %s.\n",
            msgNum,
            s,
            session->FileName,
            lines2str(nlines));
