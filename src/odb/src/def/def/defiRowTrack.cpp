...
defiRow::defiRow(defrData* data) : defData(data)
{
  Init();
}
...
void defiRow::setup(const char* name,
                    const char* macro,
                    double x,
                    double y,
                    int orient)
...
void defiRow::setDo(double x_num, double y_num, double x_step, double y_step)
{
  xStep_ = x_step;
  yStep_ = y_step;
  xNum_ = x_num;
...
void defiRow::addNumProperty(const char* name,
                             const double d,
                             const char* value,
                             const char type)
{
...
defiTrack::defiTrack(defrData* data) : defData(data)
{
  Init();
}
...
defiGcellGrid::defiGcellGrid(defrData* data) : defData(data)
{
  Init();
}
