...
std::set<dbVia*> lefout::writeBoxes(dbBlock* block,
...
void lefout::writeBlock(dbBlock* db_block)
{
  ...
  const std::set<dbVia*> vias = writePins(db_block);
  _out = orig;

  // Write vias after pins so that they are defined before they are used in the
  // pin
  for (dbVia* via : vias) {
    writeBlockVia(db_block, via);
  }

  *_out << header_stream.str();
  writeObstructions(db_block);
  fmt::print(*_out, "END {}\n", db_block->getName().c_str());
}

std::set<dbVia*> lefout::writePins(dbBlock* db_block)
{
  std::set<dbVia*> vias;

  const auto power_vias = writePowerPins(db_block);
  vias.insert(power_vias.begin(), power_vias.end());

  const auto block_vias = writeBlockTerms(db_block);
  vias.insert(block_vias.begin(), block_vias.end());

  return vias;
}

std::set<dbVia*> lefout::writeBlockTerms(dbBlock* db_block)
{
  std::set<dbVia*> vias;
  for (dbBTerm* b_term : db_block->getBTerms()) {
    ...
    for (dbBPin* db_b_pin : b_term->getBPins()) {
      ...
      const auto boxvias = writeBoxes(db_block, term_pins, "      ");
...
std::set<dbVia*> lefout::writePowerPins(dbBlock* db_block)
{
  std::set<dbVia*> vias;
  ...
  for (dbNet* net : db_block->getNets()) {
    ...
    for (dbSWire* special_wire : net->getSWires()) {
      ...
      const auto boxvias = writeBoxes(db_block, wires, /*indent=*/"      ");
      vias.insert(boxvias.begin(), boxvias.end());
      fmt::print(*_out, "    END\n");
    }
    fmt::print(*_out, "  END {}\n", net->getName().c_str());
