...
odb::OdbPtrMap<dbTechLayer, odb::dbTechVia*> dbBlock::getDefaultVias()
{
  odb::dbTech* tech = getTech();
  odb::dbSet<odb::dbTechVia> vias = tech->getVias();
  odb::OdbPtrMap<dbTechLayer, odb::dbTechVia*> default_vias;
...
{
  dbSet<dbGlobalConnect> gcs = getGlobalConnects();
  odb::OdbPtrSet<dbGlobalConnect> connects(gcs.begin(), gcs.end());
                            ...
                            bool verbose)
{
  ...
  odb::OdbPtrSet<dbITerm> connected_iterms;
  odb::OdbPtrSet<dbITerm> skipped_iterms;
  // only search for instances once
  std::map<std::string, std::vector<dbInst*>> inst_map;
  odb::OdbPtrSet<dbInst> donottouchinsts;
  for (dbGlobalConnect* connect : connects) {
    ...
    odb::OdbPtrSet<dbInst> remove_insts;
