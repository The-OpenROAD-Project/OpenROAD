...
# Return value is currently NULL, better to be an empty list
check "block gcell grid" {$block getGCellGrid} "NULL"

set die_area_rect [$block getDieArea]
check "block die area" {list [$die_area_rect xMin] [$die_area_rect yMin] \
                            [$die_area_rect xMax] [$die_area_rect yMax]} "0 0 200260 201600"
...
check "row bbox" {list [[$row getBBox] xMin] [[$row getBBox] yMin] [[$row getBBox] xMax] \
                      [[$row getBBox] yMax]} "20140 22400 180500 25200" ;# Guess at the xMax value
...
# Return value is currently NULL, better to be an empty list
check "instance halo" {$inst getHalo} "NULL"
...
# Return value is currently NULL, better to be an empty list
check "master logically equiv" {$master getLEQ} "NULL"
# Return value is currently NULL, better to be an empty list
check "master electrially equiv" {$master getEEQ} "NULL"
check "master symmetry" {list [$master getSymmetryX] [$master getSymmetryY] \
                             [$master getSymmetryR90]} "1 1 0"
...
check "master placement boundary" {set rect [$master getPlacementBoundary]; list [$rect xMin] \
                                       [$rect yMin] [$rect xMax] [$rect yMax]} "0 0 760 2800"
...
# Return value is currently NULL, better to be an empty list
check "net wires" {$net getWire} "NULL"
check "net swires" {$net getSWires} ""
# Return value is currently NULL, better to be an empty list
check "net global wire" {$net getGlobalWire} "NULL"
# Return value is currently NULL, better to be an empty list
