...
class TestBlock(odbUnitTest.TestCase):
    ...
    def test_find(self):
        ...
        # child
        self.assertEqual(
            self.parentBlock.findChild("2LevelBlock").getName(), "2LevelBlock"
        )
    ...
    def block_placement(self, test_num, flag):
        ...
        if (flag and test_num == 3) or (not flag and test_num >= 3):
            odb.dbObstruction_create(
                self.block, self.lib.getTech().findLayer("L1"), -1500, 0, -1580, 50
            )
