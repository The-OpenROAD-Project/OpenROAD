...
export class TclCompleter {
  ...
  handleKeyDown(e) {
    ...
    if (e.key === 'Tab') {
      ...
      if (popupVisible) {
        ...
        this._requestCompletions(this._input.value, this._input.selectionStart);
      }
      return true;
    }
    ...
    if (e.key === 'ArrowDown') {
      ...
      if (this._historyIndex >= 0) {
        if (this._input.value.substring(this._input.selectionStart)
                .indexOf('\n') !== -1) {
          return false;
        }
        e.preventDefault();
    ...
    if (e.key === 'ArrowUp') {
      ...
      if (this._historyIndex < this._history.length - 1) {
        if (this._input.value.substring(0, this._input.selectionStart)
                .indexOf('\n') !== -1) {
          return false;
        }
        e.preventDefault();
  ...
  _onInput() {
    ...
    this._debounceTimer = setTimeout(() => {
      ...
      const shouldTrigger = prefix.length >= kMinPrefixLength ||
  ...
  _requestCompletions(line, cursorPos) {
    ...
    this._ws
        ...
        .then(data => {
          ...
          this._showPopup(
              data.completions, data.replace_start, data.replace_end);
        })
        .catch(() => {
          this._hidePopup();
        });
  ...
  _highlightSelected() {
    ...
    if (children[this._selectedIndex] &&
        children[this._selectedIndex].scrollIntoView) {
