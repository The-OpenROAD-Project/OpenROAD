...
import {applyGLTheme, getCookie, setCookie} from './theme.js';
...
function updateStatus() {
  ...
  if (!isConnected) {
    ...
    if (!disconnectTimeout) {
      disconnectTimeout = setTimeout(() => {
        if (!app.websocketManager?.isConnected) {
          statusDiv.innerHTML =
              '<div class="disconnected-banner">⚠ OpenROAD disconnected — retrying…</div>';
    ...
    if (pendingCount === 0) {
      ...
      statusDiv.innerHTML =
          `<div class="pending-indicator">pending: ${pendingCount}</div>`;
      statusDiv.style.display = 'block';
...
const WebSocketTileLayer =
    createWebSocketTileLayer(visibility, app.visibleLayers);
const BLANK_TILE =
...
function createLayoutViewer(container) {
  ...
  function updateScaleBar() {
    ...
    let label;
    if (niceUm >= 1000)
      label = (niceUm / 1000) + ' mm';
    else if (niceUm >= 1)
      label = niceUm + ' \u00b5m';
    else if (niceUm >= 0.001)
      label = (niceUm * 1000) + ' nm';
    else
      label = (niceUm * 1e6) + ' pm';
  ...
  app.rulerManager =
      new RulerManager(app, visibility, updateInspector, focusComponent);
}
...
function isTclComplete(cmd) {
  let braces = 0, brackets = 0, quotes = 0;
  let escaped = false;
...
function createTclConsole(container) {
  ...
  el.innerHTML = '<div class="tcl-output"></div>' +
      '<div class="tcl-input-row">' +
      '  <span class="tcl-prompt">%</span>' +
      '  <textarea class="tcl-input" rows="1" placeholder="Enter Tcl command..." spellcheck="false" autocomplete="off" autocapitalize="none" autocorrect="off"></textarea>' +
      '</div>';
  container.element.appendChild(el);
  ...
  input.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'Enter') {
      if (e.shiftKey || !isTclComplete(input.value)) {
        return;
      }
      e.preventDefault();
...
function createHelpWidget(container) {
  ...
  el.innerHTML = '<h3>Keyboard Shortcuts</h3>' +
      '<table>' +
...
function createSelectHighlight(container) {
  createStubPanel(container, 'Selection', 'Selection and highlight browser.');
}
...
function createStubPanel(container, title, description) {
  ...
  el.innerHTML = `<div class="stub-title">${title}</div>` +
...
app.goldenLayout.registerComponentFactoryFunction(
    'LayoutViewer', createLayoutViewer);
app.goldenLayout.registerComponentFactoryFunction(
    'DisplayControls', createDisplayControls);
app.goldenLayout.registerComponentFactoryFunction(
    'TclConsole', createTclConsole);
...
app.goldenLayout.registerComponentFactoryFunction(
    'TimingWidget', createTimingWidget);
app.goldenLayout.registerComponentFactoryFunction('DRCWidget', createDRCWidget);
app.goldenLayout.registerComponentFactoryFunction(
    'ClockWidget', createClockWidget);
app.goldenLayout.registerComponentFactoryFunction(
    'ChartsWidget', createChartsWidget);
app.goldenLayout.registerComponentFactoryFunction(
    'SchematicWidget', createSchematicWidget);
app.goldenLayout.registerComponentFactoryFunction(
    'HelpWidget', createHelpWidget);
app.goldenLayout.registerComponentFactoryFunction(
    'SelectHighlight', createSelectHighlight);
...
app.goldenLayout.on('stateChanged', () => {
  localStorage.setItem(
      'gl-layout', JSON.stringify(app.goldenLayout.saveLayout()));
});

// Handle window resize
window.addEventListener('resize', () => {
  const menuBarHeight = document.getElementById('menu-bar').offsetHeight;
  app.goldenLayout.setSize(
      window.innerWidth, window.innerHeight - menuBarHeight);
});
...
function ensureDebugContinueButton() {
  ...
  btn.addEventListener('click', () => {
    ...
    // actually resumed.
    app.websocketManager.request({type: 'debug_continue'}).catch(() => {});
  });
...
app.websocketManager.onPush = (msg) => {
  if (msg.type === 'refresh') {
    ...
    app.websocketManager._shutdown = true;
    statusDiv.innerHTML =
        '<div class="disconnected-banner">Server stopped</div>';
    statusDiv.style.display = 'block';
  }
};
...
app.websocketManager.readyPromise.then(async () => {
  try {
    ...
    populateDisplayControls(
        app, visibility, WebSocketTileLayer, techData, redrawAllLayers,
        HeatMapTileLayer);
