...
def inline_script_hashes(assets):
    ...
    An inline block with no matching hash is simply not executed.
    """
    hashes = []
    pages = 0
    for served, _, _, data in assets:
        ...
        pages += 1
        if pages > 1:
            # One policy is served with every response, so a second page's
            # hashes would widen what the first may execute.
            raise SystemExit(
