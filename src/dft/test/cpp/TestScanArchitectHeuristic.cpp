...
BOOST_AUTO_TEST_CASE(test_one_clock_domain_no_mix) {
  ...
  for (uint64_t i = 0; i < 20; ++i) {
    ...
    scan_cells.push_back(std::make_shared<ScanCellMock>(ss.str(), ClockDomain("clk1", ClockEdge::Rising)));
  }

  std::unique_ptr<ScanCellsBucket> scan_cells_bucket = std::make_unique<ScanCellsBucket>();
  scan_cells_bucket->init(config, scan_cells);

  std::unique_ptr<ScanArchitect> scan_architect = ScanArchitect::ConstructScanScanArchitect(config, std::move(scan_cells_bucket));
  scan_architect->init();
  scan_architect->architect();
  std::vector<std::unique_ptr<ScanChain>> scan_chains = scan_architect->getScanChains();
  ...
  for (const std::string& name: scan_cell_names) {
    const bool test = scan_cells_in_chains_names.find(name) != scan_cells_in_chains_names.end();
...
BOOST_AUTO_TEST_CASE(test_two_clock_domain_no_mix) {
  ...
  for (uint64_t i = 0; i < 15; ++i) {
    ...
    ++name_number;
    scan_cell_names.push_back(ss.str());
    scan_cells.push_back(std::make_shared<ScanCellMock>(ss.str(), ClockDomain("clk2", ClockEdge::Rising)));
  }
...
BOOST_AUTO_TEST_CASE(test_two_edges_no_mix) {
  ...
  for (uint64_t i = 0; i < 15; ++i) {
    ...
    ++name_number;
    scan_cell_names.push_back(ss.str());
    scan_cells.push_back(std::make_shared<ScanCellMock>(ss.str(), ClockDomain("clk1", ClockEdge::Falling)));
  }
