...
odb::dbInst* CreateTmpCell(const char* name, const char* lib_name,
                           const char* master_name, odb::dbDatabase* db) {
...
TEST(MBFFTest, FlopsCanBeIdentifiedAsATrayAndNot) {
  ...
  ord->readDef("src/gpl/test/library/test/test0.def",
               db->getTech(), /*continue_on_errors=*/false,
               /*floorplan_init=*/false, /*incremental=*/false,
               /*child=*/false);

  MBFF mbff(db, sta, log, rsz, /*threads=*/1, /*multistart=*/20,
            /*num_paths=*/0, /*gui_debug=*/false);
