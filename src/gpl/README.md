...
| -place_ios | Flag to co-optimize movable IO pins with cells in the same placement solve. The resulting pin locations are written to the database but are not legalized to routing tracks; place_pins must be run afterwards. Cannot be combined with -timing_driven, -routability_driven, -incremental, -skip_io, or -skip_nesterov_place. The default value is False, allowed values are boolean. |
