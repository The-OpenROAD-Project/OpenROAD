...
void RouteBase::init()
{
  ...
  tg_->setLogger(log_);
  minRcCellSize_.resize(nbc_->nbcGCells().size(), std::make_pair(0, 0));
}
...
std::pair<bool, bool> RouteBase::routability()
{
  ...
  if ((minRc_ - curRc) > 0.001) {
    ...
    // save cell size info
    for (auto& gCell : nbc_->nbcGCells()) {
      if (!gCell->isStdInstance()) {
        continue;
      }

      minRcCellSize_[&gCell - nbc_->nbcGCells().data()]
...
void RouteBase::revertGCellSizeToMinRc()
{
  ...
  for (auto& gCell : nbc_->nbcGCells()) {
    ...
    int idx = &gCell - nbc_->nbcGCells().data();
