...
#pragma omp for
    for (auto gNet = gNetStor_.begin(); gNet < gNetStor_.end(); ++gNet) {
      ...
      #pragma omp simd
      for (int i = 0; i < numPins; i++) {
        expMinX_fast[i] = fastExp(expMinX[i]);
        expMaxX_fast[i] = fastExp(expMaxX[i]);
        expMinY_fast[i] = fastExp(expMinY[i]);
        expMaxY_fast[i] = fastExp(expMaxY[i]);
      }
      ...
      for (int i = 0; i < numPins; i++) {
        ...
        if (expMinX[i] > minBar) {
          ...
          if (debug_enabled && gPin->getGCell() && gPin->getGCell()->isInstance()) {
            debugPrint(log_, GPL, "wlUpdateWA", 1, "MinX updated: {} {:g}", gPin->getGCell()->getName(), gPin->minExpSumX());
          }
        }
        if (expMaxX[i] > minBar) {
          ...
          if (debug_enabled && gPin->getGCell() && gPin->getGCell()->isInstance()) {
            debugPrint(log_, GPL, "wlUpdateWA", 1, "MaxX updated: {} {:g}", gPin->getGCell()->getName(), gPin->maxExpSumX());
          }
        }
        if (expMinY[i] > minBar) {
          ...
          if (debug_enabled && gPin->getGCell() && gPin->getGCell()->isInstance()) {
            debugPrint(log_, GPL, "wlUpdateWA", 1, "MinY updated: {} {:g}", gPin->getGCell()->getName(), gPin->minExpSumY());
          }
        }
        if (expMaxY[i] > minBar) {
          ...
          if (debug_enabled && gPin->getGCell() && gPin->getGCell()->isInstance()) {
            debugPrint(log_, GPL, "wlUpdateWA", 1, "MaxY updated: {} {:g}", gPin->getGCell()->getName(), gPin->maxExpSumY());
          }
        }
      }
    }
...
std::optional<std::pair<odb::dbInst*, size_t>> NesterovBase::destroyCbkGCell(
    odb::dbInst* db_inst)
{
  debugPrint(log_,
             GPL,
...
std::optional<std::pair<odb::dbInst*, size_t>>
NesterovBaseCommon::destroyCbkGCell(odb::dbInst* db_inst)
{
  auto it = db_inst_to_nbc_index_map_.find(db_inst);
  if (it == db_inst_to_nbc_index_map_.end()) {
    debugPrint(log_,
