...
#pragma omp parallel for num_threads(nbc_->getNumThreads())
  for (size_t i = 0; i < nb_gcells_.size(); ++i) {
    ...
    curSLPCoordi_[i] = prevSLPCoordi_[i] = curCoordi_[i]
        = initCoordi_[i] = FloatPoint(gCell->dCx(), gCell->dCy());
