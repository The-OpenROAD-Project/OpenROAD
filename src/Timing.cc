...
std::vector<Timing::ClockInfo> Timing::getClockInfo()
{
  ...
  for (const sta::Clock* clk : sta->cmdMode()->sdc()->clocks()) {
    ...
    if (clk->waveform()) {
      for (float edge : *clk->waveform()) {
        info.waveform.push_back(edge);
      }
    }
  ...
  for (auto& path_end : path_ends) {
    ...
    for (size_t i = 0; i < expand.size(); i++) {
      ...
      const bool is_rising
          = ref->transition(sta) == sta::RiseFall::rise();
      ...
      while (iter.hasNext()) {
        ...
        if (edge->isWire()) {
          ...
          if (network->isTopLevelPort(to_pin)) {
            ...
            node_fanout
                += sdc->portExtFanout(port, sta::MinMax::max()) + 1;
          } else {
            node_fanout++;
          }
        }
      ...
      if (i > 0) {
        ...
        bool pin_is_clock
            = sta->isClock(pin, sta->cmdScene()->mode());
