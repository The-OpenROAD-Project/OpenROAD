...
private:
    ...
    std::string progressFormat_ = "{:.1f}% completion -- {} wires have been extracted\n";

    float calculateCurrentPercent() const {
        return ceil(100.0f * static_cast<float>(extractedWireCount_) / totalWireCount_);
    }
    ...
    void printProgress(float currentPercent) const {
        fprintf(stdout, "%3d%% completion -- %6d wires have been extracted\n",
...
public:
    ...
    ~ScopedProgressUpdate() {
        tracker_.updateProgress(updateCount_);
    }
