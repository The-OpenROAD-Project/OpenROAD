...
class Pattern:
    ...
    def makeStats(targetPercent1, targetPercent5, capTable, sfile) :
        ...
        for s in capTable :
            ...
            if dd>targetPercent1 or dd<-targetPercent1: cnt1+=1
...
parser.add_argument('-target_error_percent', type=float, default=3.0, help='Report on error greater than abs value, default=3.0')
...
parser.add_argument('-out_dir', type=str, default='stats_diff_spef', help='directory name for all reports, default=stats_diff_spef')
...
rows = []
for line in f:
    if line.find(cntx)!=-1: continue
    if line.find('netCcap')!=-1: continue
    if line.find('ccCap')!=-1: continue
    if line.find('netGndCap')!=-1: continue
...
for k in capDict.keys(): #netCap, resRes 
    if k=='netCcap' : continue 
    if k=='netGndCap' : continue 
...
for p in patDict.keys(): 
    ...
    for wireNum in range(6):
        ...
        for resCap in capDict.keys(): 
            if resCap=='netCcap' : continue 
            if resCap=='netGndCap' : continue
            ...
            outliersFile=dirPath + "/" + p + "." + str(wireNum) + "." + resCap + ".outliers" 
            ...
            if dbg1>0 : 
                ...
                print('wireNum=', wireNum, '  capIndex=', capIndex, '  patIndex=', patIndex) 
