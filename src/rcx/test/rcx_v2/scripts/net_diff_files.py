...
class DiffPlot :
    def Plot3x3(self, xyBuckets, xyBuckets2, wl, units, suptitle):
        ...
        fig, axs = plt.subplots(rowCnt, colCnt, figsize=(10, 10))  # Adjust figsize as needed
...
class Bucket :
    ...
    def Print(_, dd, ii) : 
        if ii==_.last-1 : 
            ...
            print (dd, " ", _.value[ii], " " ,_.value[ii+1], " ", ii, " cnt=" , _.cnt[ii])

    def Update(_, dd, ii) : 
        _.cnt[ii] = _.cnt[ii]+1 
        if _.dbg>0 : _.Print(dd, ii)
...
class Diff :
    ...
    def ReadMinMax(_, fileName, row_filter, WL, Val, adjust_percent=1) :
        ...
        for line in f:
            ...
            if WL[0]> wlen : WL[0]= wlen 
            if WL[1]< wlen : WL[1]= wlen

            row[6]= refVal 
            if Val[0]> refVal : Val[0]= refVal 
            if Val[1]< refVal : Val[1]= refVal
...
parser = argparse.ArgumentParser(description='Diff_Spef_out Total Net Resistance ot Capacitance Error Distribution')
parser.add_argument('-rc_value', type=str, default='netCap', help='values should be netCap or netRes, default=netCap')
parser.add_argument('-WL_val', type=int, default=1, help='bucket based ditsribution based on 0: WireLoad or 1:Value, default=1')
parser.add_argument('-corner', type=int, default=0, help='target extraction corner, default=0')
#parser.add_argument('buckets')
parser.add_argument('-name', type=str, default='All',  help='all rows or Substring of net Name, default=All')
parser.add_argument('-suptitle', type=str, default='Error Distribution', help='Title of Graph, default=Error Distribution')
parser.add_argument('-diff_spef_out_file', type=str, default='diff_spef.out', help='diff_spef.out file after running diff_spef command, default=diff_spef.out')
parser.add_argument('-diff_spef_out_file_2', type=str, default='', help='comparison diff_spef.out file after running diff_spef command, default=empty')
