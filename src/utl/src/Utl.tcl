...
proc check_ip { args } {
  sta::parse_key_args "check_ip" args \
    keys {-master -max_polygons -report_file} \
    flags {-all -verbose}
  ...
  if { !$check_all && $master_name eq "" } {
    utl::error IPC 10 "Must specify either -master or -all"
  }
  ...
  return [utl::check_ip_cmd $master_name $check_all $max_polygons \
    $report_file $verbose]
}
