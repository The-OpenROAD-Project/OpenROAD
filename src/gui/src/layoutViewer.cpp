...
LayoutViewer::LayoutViewer(
    Options *options, ScriptWidget *output_widget, const SelectionSet &selected,
    ...
    const std::set<odb::dbNet *> &net_tracks, Gui *gui,
    const std::function<bool()> &using_dbu,
    const std::function<bool()> &show_ruler_as_euclidian,
    const std::function<bool()> &show_db_view, QWidget *parent)
    : QWidget(parent), chip_(nullptr), options_(options),
      output_widget_(output_widget), selected_(selected),
      highlighted_(highlighted), rulers_(rulers), labels_(labels),
      scroller_(nullptr), pixels_per_dbu_(1.0), fit_pixels_per_dbu_(1.0),
      min_depth_(0), max_depth_(99), rubber_band_showing_(false),
      is_view_dragging_(false), gui_(gui), using_dbu_(using_dbu),
      show_ruler_as_euclidian_(show_ruler_as_euclidian),
      show_db_view_(show_db_view), modules_(module_settings),
      building_ruler_(false), ruler_start_(nullptr), snap_edge_showing_(false),
      animate_selection_(nullptr), repaint_requested_(false), logger_(nullptr),
      layout_context_menu_(new QMenu(tr("Layout Menu"), this)),
      focus_nets_(focus_nets), route_guides_(route_guides),
      net_tracks_(net_tracks), viewer_thread_(this) {
  ...
  connect(&viewer_thread_, &RenderThread::done, this,
          &LayoutViewer::updatePixmap);

  connect(&loading_timer_, &QTimer::timeout, this,
...
Rect LayoutViewer::getPaddedRect(const Rect &rect, double factor) {
  ...
  return Rect(rect.xMin() - margin, rect.yMin() - margin, rect.xMax() + margin,
              rect.yMax() + margin);
}

qreal LayoutViewer::computePixelsPerDBU(const QSize &size,
                                        const Rect &dbu_rect) {
...
void LayoutViewer::setPixelsPerDBU(qreal pixels_per_dbu) {
  ...
  const Rect current_viewer(0, 0, this->size().width() / pixels_per_dbu_,
                            this->size().height() / pixels_per_dbu_);

  // ensure max size is not exceeded
  qreal maximum_pixels_per_dbu =
      0.98 * computePixelsPerDBU(QSize(QWIDGETSIZE_MAX, QWIDGETSIZE_MAX),
                                 current_viewer);
...
odb::Point LayoutViewer::getVisibleCenter() { return center_; }
...
bool LayoutViewer::compareEdges(const Edge &lhs, const Edge &rhs) const {
  const uint64_t lhs_length_sqrd = std::pow(lhs.first.x() - lhs.second.x(), 2) +
                                   std::pow(lhs.first.y() - lhs.second.y(), 2);
  const uint64_t rhs_length_sqrd = std::pow(rhs.first.x() - rhs.second.x(), 2) +
...
void LayoutViewer::searchNearestViaEdge(
    dbTechLayer *cut_layer, dbTechLayer *search_layer, const Rect &search_line,
    const int shape_limit,
    const std::function<void(const Rect &rect)> &check_rect) {
  auto via_shapes = search_.searchSNetViaShapes(
      getBlock(), cut_layer, search_line.xMin(), search_line.yMin(),
      search_line.xMax(), search_line.yMax(), shape_limit);
...
LayoutViewer::searchNearestEdge(const odb::Point &pt, bool horizontal,
                                bool vertical) {
  if (!hasDesign()) {
    return {Edge(), false};
  }
  ...
  auto check_rect = [this, &pt, &closest_edge, &edge_distance, &horizontal,
  ...
  if (horizontal) {
    search_line = odb::Rect(pt.x(), pt.y() - search_radius, pt.x(),
                            pt.y() + search_radius);
  } else if (vertical) {
    search_line = odb::Rect(pt.x() - search_radius, pt.y(),
                            pt.x() + search_radius, pt.y());
  } else {
    search_line = odb::Rect(pt.x() - search_radius, pt.y() - search_radius,
                            pt.x() + search_radius, pt.y() + search_radius);
  }

  auto inst_range = search_.searchInsts(
      getBlock(), search_line.xMin(), search_line.yMin(), search_line.xMax(),
      search_line.yMax(), instanceSizeLimit());
  ...
  for (auto layer : tech->getLayers()) {
    ...
    for (auto inst : insts) {
      ...
      if (inst_osb_visible) {
        for (auto &box : inst_boxes->obs) {
          ...
          odb::Rect trans_box(rect.left(), rect.bottom(), rect.right(),
                              rect.top());
    ...
    if (routing_visible || vias_visible || io_pins_visible) {
      auto box_shapes = search_.searchBoxShapes(
          getBlock(), layer, search_line.xMin(), search_line.yMin(),
          search_line.xMax(), search_line.yMax(), shape_limit);
    ...
    if (options_->areSpecialRoutingViasVisible()) {
      if (layer->getType() == dbTechLayerType::CUT) {
        searchNearestViaEdge(layer, layer, search_line, shape_limit,
                             check_rect);
      } else {
        if (auto upper = layer->getUpperLayer()) {
          searchNearestViaEdge(upper, layer, search_line, shape_limit,
                               check_rect);
        }
        if (auto lower = layer->getLowerLayer()) {
          searchNearestViaEdge(lower, layer, search_line, shape_limit,
                               check_rect);
        }
      }
    }
    ...
    if (options_->areSpecialRoutingSegmentsVisible()) {
      auto polygon_shapes = search_.searchSNetShapes(
          getBlock(), layer, search_line.xMin(), search_line.yMin(),
          search_line.xMax(), search_line.yMax(), shape_limit);
    ...
    if (options_->areFillsVisible()) {
      auto fills = search_.searchFills(getBlock(), layer, search_line.xMin(),
                                       search_line.yMin(), search_line.xMax(),
                                       search_line.yMax(), shape_limit);
      for (auto *fill : fills) {
        odb::Rect box;
        fill->getRect(box);
        check_rect(box);
  ...
  if (options_->areBlockagesVisible()) {
    auto blcks = search_.searchBlockages(getBlock(), search_line.xMin(),
                                         search_line.yMin(), search_line.xMax(),
                                         search_line.yMax(), shape_limit);
    for (auto *blck : blcks) {
  ...
  const std::array<odb::Point, 3> snap_points = {
      ...
      odb::Point((closest_edge.first.x() + closest_edge.second.x()) / 2,
                 (closest_edge.first.y() + closest_edge.second.y()) /
                     2),  // midpoint
...
void LayoutViewer::selectViaShapesAt(dbTechLayer *cut_layer,
                                     ...
                                     const Rect &region, const int shape_limit,
                                     std::vector<Selected> &selections) {
  auto via_shapes = search_.searchSNetViaShapes(
      getBlock(), cut_layer, region.xMin(), region.yMin(), region.xMax(),
      region.yMax(), shape_limit);
...
void LayoutViewer::selectAt(odb::Rect region,
  ...
  if (block && options_->areBlockagesVisible() &&
    ...
    auto blockages =
        search_.searchBlockages(block, region.xMin(), region.yMin(),
                                region.xMax(), region.yMax(), shape_limit);
  ...
  for (auto layer : rev_layers) {
    ...
    if (options_->areObstructionsVisible() &&
      ...
      auto obs =
          search_.searchObstructions(block, layer, region.xMin(), region.yMin(),
                                     region.xMax(), region.yMax(), shape_limit);
      for (auto *ob : obs) {
    ...
    if (routing_visible || vias_visible || io_pins_visible) {
      auto box_shapes =
          search_.searchBoxShapes(block, layer, region.xMin(), region.yMin(),
                                  region.xMax(), region.yMax(), shape_limit);
    ...
    if (options_->areSpecialRoutingSegmentsVisible()) {
      auto polygon_shapes =
          search_.searchSNetShapes(block, layer, region.xMin(), region.yMin(),
                                   region.xMax(), region.yMax(), shape_limit);
  ...
  if (block) {
    ...
    auto insts =
        search_.searchInsts(block, region.xMin(), region.yMin(), region.xMax(),
                            region.yMax(), instanceSizeLimit());
    ...
    if (options_->areIOPinsVisible() && options_->areIOPinsSelectable()) {
      for (auto *layer : block->getTech()->getLayers()) {
        ...
        for (const auto &[box, bpin] :
             search_.searchBPins(block, layer, region.xMin(), region.yMin(),
                                 region.xMax(), region.yMax(), shape_limit)) {
...
void LayoutViewer::mousePressEvent(QMouseEvent *event) {
  ...
  if (building_ruler_ && event->button() == Qt::LeftButton) {
    ...
    if (ruler_start_ == nullptr) {
      ...
      emit addRuler(ruler_start_->x(), ruler_start_->y(), next_ruler_pt.x(),
                    next_ruler_pt.y());
      cancelRulerBuild();
    }
  }
        ...
        if (box->isVia()) {
          ...
          for (auto *via_box : tech_via->getBoxes()) {
            ...
            boxes[layer].mterms[mterm].emplace_back(
                QRect{box_rect.xMin(), box_rect.yMin(),
...
LayoutViewer::getRowRects(odb::dbBlock *block, const odb::Rect &bounds) {
  ...
  auto rows =
      search_.searchRows(block, bounds.xMin(), bounds.yMin(), bounds.xMax(),
                         bounds.yMax(), min_resolution_row);
...
void LayoutViewer::selectionAnimation(const Selected &selection, int repeats,
                                      int update_interval) {
  ...
  if (selection) {
    ...
    animate_selection_ = std::make_unique<AnimatedSelected>(
        AnimatedSelected{selection, 0, state_reset_interval * repeats,
                         state_reset_interval, nullptr});
    ...
    const qint64 max_animate_time =
        ...
        (animate_selection_->max_state_count + 2) * (qint64)update_interval;
    connect(animate_selection_->timer.get(), &QTimer::timeout,
            [this, max_animate_time]() {
              ...
              if (animate_selection_->max_state_count !=
                      0 && // if max_state_count == 0 animate until new
...
Rect LayoutViewer::screenToDBU(const QRectF &screen_rect) const {
  int dbu_left =
      (int)floor((screen_rect.left() - centering_shift_.x()) / pixels_per_dbu_);
  int dbu_right =
      (int)ceil((screen_rect.right() - centering_shift_.x()) / pixels_per_dbu_);
  // Flip the y-coordinate (see file level comments)
  int dbu_top =
      (int)floor((centering_shift_.y() - screen_rect.top()) / pixels_per_dbu_);
...
void LayoutViewer::drawScaleBar(QPainter *painter, const QRect &rect) {
  ...
  const QRectF scale_bar_outline(rect.left() + 10, rect.bottom() - 20,
...
void LayoutViewer::drawLoadingIndicator(QPainter *painter,
                                        const QRect &bounds) {
  ...
  painter->drawText(background.left(), background.bottom(),
...
QRect LayoutViewer::computeIndicatorBackground(QPainter *painter,
  ...
  const QRect background(bounds.left() + 2 /* px */, bounds.top() + 2 /* px */,
...
void LayoutViewer::paintEvent(QPaintEvent *event) {
  ...
  GuiPainter gui_painter(&painter, options_, screenToDBU(draw_bounds),
                         pixels_per_dbu_, getDbuPerMicron());
  ...
  for (const auto &label : labels_) {
    ...
    label->setOutline(
        gui_painter.stringBoundaries(label->getPt().x(), label->getPt().y(),
                                     label->getAnchor(), label->getText()));
    painter.restore();
  }
  ...
  if (animate_selection_ != nullptr) {
    ...
    const int pen_width =
        animate_selection_->state_count % animate_selection_->state_modulo + 1;
    if (pen_width == 1) {
    ...
    if (draw_hightlight) {
      animate_selection_->selection.highlight(gui_painter, Painter::kHighlight,
                                              pen_width, brush);
    }
  }
  ...
  if (building_ruler_ && ruler_start_ != nullptr) {
    ...
    gui_painter.drawRuler(ruler_start_->x(), ruler_start_->y(),
                          snapped_mouse_pos.x(), snapped_mouse_pos.y(),
  ...
  if (snap_edge_showing_) {
    ...
    if (snap_edge_.first == snap_edge_.second) {
      painter.drawEllipse(QPointF(snap_edge_.first.x(), snap_edge_.first.y()),
                          5.0 / pixels_per_dbu_, 5.0 / pixels_per_dbu_);
    } else {
      painter.drawLine(
...
void LayoutViewer::selectHighlightConnectedNets(bool select_flag, bool output,
                                                bool input) {
  int highlight_group = 0;
  if (!select_flag) {
    HighlightGroupDialog dlg;
...
void LayoutViewer::chipLoaded(odb::dbChip *chip) { search_.setTopChip(chip); }

void LayoutViewer::setScroller(LayoutScroll *scroller) {
  scroller_ = scroller;

  // ensure changes in the scroll area are announced to the layout viewer
  connect(scroller_, &LayoutScroll::viewportChanged, this,
          &LayoutViewer::viewportUpdated);
  connect(scroller_, &LayoutScroll::centerChanged, this,
...
QImage LayoutViewer::createImage(const Rect &region, int width_px,
  ...
  const QRegion save_region =
      QRegion(screen_region.left(), screen_region.top(), screen_region.width(),
  ...
  if (img_size != initial_size) {
    ...
    const QRegion adjusted_save_region =
        QRegion(adjuted_screen_region.left(), adjuted_screen_region.top(),
                adjuted_screen_region.width(), adjuted_screen_region.height());
  ...
  viewer_thread_.draw(img, bounding_rect, selected_, highlighted_, rulers_,
                      labels_, render_ratio, options_->background());
...
void LayoutViewer::saveImage(const QString &filepath, const Rect &region,
                             int width_px, double dbu_per_pixel) {
  if (!hasDesign()) {
    return;
  }
...
void LayoutViewer::addMenuAndActions() {
  ...
  connect(menu_actions_[kSelectConnectedInstAct], &QAction::triggered,
          [this]() { selectHighlightConnectedInst(true); });
  connect(menu_actions_[kSelectOutputNetsAct], &QAction::triggered,
          [this]() { selectHighlightConnectedNets(true, true, false); });
  connect(menu_actions_[kSelectInputNetsAct], &QAction::triggered,
          [this]() { selectHighlightConnectedNets(true, false, true); });
  connect(menu_actions_[kSelectAllNetsAct], &QAction::triggered,
          [this]() { selectHighlightConnectedNets(true, true, true); });
  connect(menu_actions_[kSelectAllBufferTreesAct], &QAction::triggered,
          [this]() { selectHighlightConnectedBufferTrees(true); });

  connect(menu_actions_[kHighlightConnectedInstAct], &QAction::triggered,
          [this]() { selectHighlightConnectedInst(false); });
  connect(menu_actions_[kHighlightOutputNetsAct], &QAction::triggered,
          [this]() { selectHighlightConnectedNets(false, true, false); });
  connect(menu_actions_[kHighlightInputNetsAct], &QAction::triggered,
          [this]() { this->selectHighlightConnectedNets(false, false, true); });
  connect(menu_actions_[kHighlightAllNetsAct], &QAction::triggered,
          [this]() { selectHighlightConnectedNets(false, true, true); });
  connect(menu_actions_[kHighlightAllBufferTreesAct0], &QAction::triggered,
          [this]() { selectHighlightConnectedBufferTrees(false, 0); });
  connect(menu_actions_[kHighlightAllBufferTreesAct1], &QAction::triggered,
          [this]() { selectHighlightConnectedBufferTrees(false, 1); });
  connect(menu_actions_[kHighlightAllBufferTreesAct2], &QAction::triggered,
          [this]() { selectHighlightConnectedBufferTrees(false, 2); });
  connect(menu_actions_[kHighlightAllBufferTreesAct3], &QAction::triggered,
          [this]() { selectHighlightConnectedBufferTrees(false, 3); });

  connect(menu_actions_[kViewZoominAct], &QAction::triggered,
          [this]() { zoomIn(); });
  connect(menu_actions_[kViewZoomoutAct], &QAction::triggered,
          [this]() { zoomOut(); });
  connect(menu_actions_[kViewZoomfitAct], &QAction::triggered,
          [this]() { fit(); });

  connect(menu_actions_[kSaveVisibleImageAct], &QAction::triggered,
          [this]() { saveImage(""); });
...
void LayoutViewer::generateCutLayerMaximumSizes() {
  ...
  for (auto layer : tech->getLayers()) {
    if (layer->getType() == dbTechLayerType::CUT) {
        ...
        for (auto via : tech->getVias()) {
          for (auto box : via->getBoxes()) {
            if (box->getTechLayer() == layer) {
              width = std::max(width, static_cast<int>(std::max(box->getDX(),
                                                                box->getDY())));
            }
          }
        }
          ...
          for (dbMTerm *term : master->getMTerms()) {
            for (dbMPin *pin : term->getMPins()) {
              for (dbBox *box : pin->getGeometry()) {
                if (box->getTechLayer() == layer) {
                  width = std::max(width, static_cast<int>(std::max(
                                              box->getDX(), box->getDY())));
                }
              }
            }
      ...
      debugPrint(logger_, GUI, "cut_size", 1, "Cut size for layer {} is {}",
                 layer->getName(), width);
    }
  }
}
...
void LayoutViewer::executionPaused() { paused_ = true; }
...
LayoutScroll::LayoutScroll(
    LayoutViewer *viewer, const std::function<bool()> &default_mouse_wheel_zoom,
    const std::function<int()> &arrow_keys_scroll_step, QWidget *parent)
    : QScrollArea(parent), default_mouse_wheel_zoom_(default_mouse_wheel_zoom),
      arrow_keys_scroll_step_(arrow_keys_scroll_step), viewer_(viewer),
...
bool LayoutScroll::isScrollingWithCursor() { return scrolling_with_cursor_; }
