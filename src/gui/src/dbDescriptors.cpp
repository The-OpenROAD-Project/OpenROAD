    ...
    odb::dbScanInst* scan_inst) const
{
  ...
  props.push_back(
      {"Clock Edge",
       scan_inst->getClockEdge() == odb::dbScanInst::ClockEdge::Rising
           ? "Rising"
           : "Falling"});

  /*
  TODO: Understand the exception thrown by the following line.
  props.push_back(
      getScanPinProperty("Scan Enable", scan_inst->getScanEnable()));
  */
