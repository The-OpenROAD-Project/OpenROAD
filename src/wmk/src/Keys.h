...
//
// Secret material for the watermark: drawing it, and deriving the per-stage
// keys from it.
//
// One secret is held by the owner.  Each stage gets its own key derived from
// it, so a key that leaks -- or is disclosed to prove one stage in court --
// does not expose the others, and so the same secret can protect several
// designs without the marks on one revealing the marks on another.  The design
// identifier and the nonce are what separate them.
//
// Nothing here is written to the database or persisted by the module.  The
// secret exists for as long as the command that uses it.
...
// from a predictable source is not a secret, and quietly falling back to an
...
// the same secret marks different objects.
