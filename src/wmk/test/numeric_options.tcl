...
# The bit is the key's, so the file is a claim file and not a forgery.
set bit [wmk::placement_target_bit_cmd $key absent_a absent_b]
set fh [open $claims w]
puts $fh "kind,A_name,B_name,target_bit,skipped_reason"
puts $fh "pair,absent_a,absent_b,$bit,"
close $fh
check "ordinary threshold rejects wholly missing placement evidence" {
  verify_watermark -placement_claims $claims -placement_key_hex $key -min_stages 1
} 0
foreach value { NaN nan Inf -Inf 1e999 -0.1 1.1 nonsense {} } {
  check "invalid tau '$value' cannot grant ownership" {
    catch {
      verify_watermark -placement_claims $claims -placement_key_hex $key \
        -tau $value -min_stages 1
    }
  } 1
...
foreach value { NaN Inf 0 4 1.5 nonsense {} } {
  check "min_stages must be an integer from one to three ($value)" {
    catch { verify_watermark -placement_claims $claims -placement_key_hex $key -min_stages $value }
  } 1
}
...
check "tau zero is allowed but cannot waive the evidence requirement" {
  verify_watermark -placement_claims $claims -placement_key_hex $key -tau 0 -min_stages 1
} 0
check "tau's documented one boundary remains allowed" {
  verify_watermark -placement_claims $claims -placement_key_hex $key -tau 1 -min_stages 1
} 0
