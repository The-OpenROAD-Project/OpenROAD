...
foreach { stage header rows expected } {
  ...
  cts "target_lcb,other_lcb,target_bit,skipped_reason"
    "leaf,leaf,1," "two different instances"
  cts "target_lcb,other_lcb,target_bit,skipped_reason"
    "leaf,peer,0,\nleaf,peer,0," "duplicate CTS pair"
  cts "target_lcb,other_lcb,target_bit,skipped_reason"
    "leaf,peer,0,\npeer,leaf,1,already_satisfied" "duplicate CTS pair"
} {
  ...
  close $stream
  set failed [catch {
    tee -variable diagnostic [list verify_watermark -${stage}_claims $path \
      -${stage}_key_hex $key -min_stages 1]
  } message]
