...
proc check_in_core { } {
  ...
  foreach inst [get_cells *] {
    ...
    if { ![$core intersects [odb::new_Point $x $y]] } {
      puts "[get_full_name $inst] ($x $y) outside of core\
    ...
    foreach tie_pin [get_pins -of $tie_inst -filter "direction == output"] {
      ...
      if { $net != "NULL" } {
        ...
        if { [llength $pins] + [llength $ports] != 2 } {
          puts "Warning: tie inst [get_full_name $tie_inst] has\
                          [llength $pins] connections."
        }
      }
    }
  }
...
proc report_ties { tie_cell_name } {
  ...
  foreach tie_inst $tie_insts {
    ...
    puts "[format %.1f [ord::dbu_to_microns $x]]\
              [format %.1f [ord::dbu_to_microns $y]]"
  }
}
