...
SwapArithModules::SwapArithModules(Resizer* resizer) : resizer_(resizer)
{
}

void SwapArithModules::init()
{
...
void SwapArithModules::replaceArithModules(int path_count,
                                           ...
                                           float slack_threshold)
{
  ...
  for (const auto& vertex_slack : violating_ends) {
    if (num_endpoints < path_count) {
      ...
      collectArithInstsOnPath(end_path, arithInsts);
      num_endpoints++;
    }
  }
  logger_->info(
...
void SwapArithModules::collectArithInstsOnPath(Path* path,
                                               set<dbModInst*>& arithInsts)
{
...
bool SwapArithModules::isArithInstance(Instance* inst, dbModInst*& mod_inst)
{
  mod_inst = nullptr;
  if (inst == nullptr) {
    return false;
  ...
  if (hier_depth >= MAX_HIER_DEPTH) {
    logger_->warn(RSZ,
                  161,
                  "Arithmetic instance search terminated after depth of {}",
                  MAX_HIER_DEPTH);
  }
...
bool SwapArithModules::hasArithOperatorProperty(dbModInst* mod_inst)
{
  if (!mod_inst) {
    return false;
  }
                                       ...
                                       const std::string& target)
{
  ...
  for (dbModInst* inst : insts) {
    ...
    string old_name(old_master->getName());
    string new_name;
...
void SwapArithModules::produceNewModuleName(const string& old_name,
                                            string& new_name,
                                            const string& target)
{
  ...
  if (target == "setup") {
    const string ALU_TARGET("KOGGE_STONE");
    const string MACC_TARGET("BASE");
    size_t pos;
  ...
  } else if (target == "hold" || target == "power" || target == "area") {
    logger_->error(RSZ, 155, "Target {} is not supported now", target);
  } else {
    logger_->error(RSZ, 156, "Target {} is not known", target);
  }
}
