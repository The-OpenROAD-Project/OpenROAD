...
std::pair<sta::Delay, sta::Delay> RepairTargetCollector::getEffortDelays(
    const sta::Pin* pin)
{
...
sta::Slack RepairTargetCollector::getPathSlackByIndex(
...
vector<const sta::Pin*> RepairTargetCollector::collectViolators(
...
void RepairTargetCollector::collectViolatingStartpoints()
{
  ...
  debugPrint(logger_,
             ...
             all_startpoints,
             all_startpoints > 0 ? int(violating_startpoints_.size()
                                       / double(all_startpoints) * 100)
                                 : 0);
}
...
sta::Slack RepairTargetCollector::getStartpointWns(
...
sta::Slack RepairTargetCollector::getStartpointTns(
    const sta::Pin* startpoint_pin) const
{
...
sta::Slack RepairTargetCollector::getEndpointWns(
...
sta::Slack RepairTargetCollector::getEndpointTns(
    const sta::Pin* endpoint_pin) const
{
...
void RepairTargetCollector::collectByPaths(int endPointIndex,
                                           int numEndpoints,
                                           int numPathsPerEndpoint)
{
...
set<const sta::Pin*> RepairTargetCollector::collectPinsByPathEndpoint(
    const sta::Pin* endpoint_pin,
...
vector<const sta::Pin*> RepairTargetCollector::collectViolatorsBySlackMargin(
    float slack_margin)
{
...
vector<const sta::Pin*> RepairTargetCollector::collectViolatorsByFaninTraversal(
    float slack_margin,
    ViolatorSortType sort_type)
{
...
void RepairTargetCollector::clearConsideredPins()
{
  considered_pins_.clear();
}

const std::set<const sta::Pin*>& RepairTargetCollector::getConsideredPins()
    const
{
...
sta::Vertex* RepairTargetCollector::getCurrentPoint(bool use_startpoints) const
{
  if (use_startpoints) {
...
const sta::Pin* RepairTargetCollector::getCurrentPointPin(
