    ...
    const double initial_design_area) const
{
  ...
  logger_->report(
      ...
      committer_.totalMoves(MoveType::kSizeUp)
          + committer_.totalMoves(MoveType::kReduceLoad)
...
bool OptimizationPolicy::reportRepairSummary() const
{
  ...
  const int reduce_load_moves
      = committer_.summaryCommittedMoves(MoveType::kReduceLoad);
  ...
  logger_->metric("design__instance__count__setup_buffer",
                  buffer_moves + split_load_moves);
  if (size_up_moves + reduce_load_moves + size_up_match_moves + vt_swap_moves
      > 0) {
    repaired = true;
    logger_->info(
        utl::RSZ,
        ...
        size_up_moves + size_up_match_moves + reduce_load_moves + vt_swap_moves,
        size_up_moves,
        size_up_match_moves,
        reduce_load_moves,
        vt_swap_moves);
