...
namespace par {
    ...
    class autoclusterMgr {
        ...
        private:
            ...
            void getBufferNet();
            void getBufferNetUtil(sta::Instance* inst, std::vector<std::pair<sta::Net*, sta::Net*> > &buffer_net);
            ...
            void mergeMacroUtil(std::string parent_name, int& merge_index, int std_cell_id);
            ...
            void MLPartNetUtil(sta::Instance *inst, int &src_id,  int &count, 
                               std::vector<int> &col_idx, std::vector<int> &row_idx,
            ...
            void MLPartBufferNetUtil(int &src_id,  int &count, 
                               std::vector<int> &col_idx, std::vector<int> &row_idx,
                               std::vector<double> &edge_weight, 
                               std::unordered_map<Cluster*, int> &node_map,
                               std::unordered_map<int, sta::Instance*> &idx_to_inst,
...
namespace ord {
    ...
    void dbPartitionDesign(ord::dbVerilogNetwork *network, odb::dbDatabase *db, 
