...
void autoclusterMgr::createBundledIO()
{
  ...
  int num_T1 = 0;
  int num_T3 = 0;
  for (term_map_iter = _bterm_map.begin(); term_map_iter != _bterm_map.end();
       term_map_iter++) {
...
void autoclusterMgr::createCluster(int& cluster_id)
{
  ...
  if (metric.num_macro > _max_num_macro || metric.num_inst > _max_num_inst) {
    ...
    if (glue_inst_vec.size() >= 1) {
      ...
      if (cluster->getNumInst() >= _min_num_inst
          || cluster->getNumMacro() >= _min_num_macro) {
...
void autoclusterMgr::createClusterUtil(Instance* inst, int& cluster_id)
{
  Metric metric = _logical_cluster_map[inst];
  Cluster* cluster
      = new Cluster(++cluster_id, true, string(_network->pathName(inst)));
  cluster->specifyTopInst(inst);
  ...
  if (cluster->getNumMacro() >= _max_num_macro
      || cluster->getNumInst() >= _max_num_inst) {
    _cluster_list.push_back(cluster);
    _break_cluster_list.push(cluster);
  } else if (cluster->getNumMacro() >= _min_num_macro
             || cluster->getNumInst() >= _min_num_inst) {
    _cluster_list.push_back(cluster);
  } else {
    _merge_cluster_list.push_back(cluster);
  }
...
void autoclusterMgr::calculateConnection(Instance* inst)
{
  ...
  while (net_iter->hasNext()) {
    ...
    if (find(_buffer_net_list.begin(), _buffer_net_list.end(), net)
        != _buffer_net_list.end())
      buffer_flag = true;
...
void autoclusterMgr::merge(string parent_name)
{
  ...
  if (_merge_cluster_list.size() > 0) {
    _merge_cluster_list[0]->specifyName(parent_name + string("_cluster_")
...
void autoclusterMgr::mergeUtil(string parent_name, int& merge_index)
{
  ...
  for (int i = 0; i < M; i++)
    for (int j = 0; j < N; j++) {
      unsigned int input
          = _merge_cluster_list[i]->getInputConnection(outside_vec[j]);
      unsigned int output
          = _merge_cluster_list[i]->getOutputConnection(outside_vec[j]);
  ...
  for (int i = 0; i < M; i++) {
    if (class_id[i] == i) {
      ...
      if (num_inst >= _min_num_inst || num_macro >= _min_num_macro) {
        ...
        _merge_cluster_list[i]->specifyName(parent_name + string("_cluster_")
                                            + to_string(merge_index++));
      } else {
...
void autoclusterMgr::breakCluster(Cluster* cluster_old, int& cluster_id)
{
  ...
  vector<Cluster*>::iterator vec_it
      = find(_cluster_list.begin(), _cluster_list.end(), cluster_old);
...
void autoclusterMgr::MLPart(Cluster* cluster, int& cluster_id)
{
  ...
  vector<Cluster*>::iterator vec_it
      = find(_cluster_list.begin(), _cluster_list.end(), cluster);
  _cluster_list.erase(vec_it);
  ...
  int count = 0;
  MLPartNetUtil(_network->topInstance(),
                src_id,
                count,
                col_idx,
                row_ptr,
...
void autoclusterMgr::MacroPart(Cluster* cluster_old, int& cluster_id)
{
  ...
  for (int i = 0; i < cluster_id_list.size(); i++)
    for (int j = i + 1; j < cluster_id_list.size(); j++) {
      ...
      // cout << "Add virtual weight:   " <<
      // _cluster_map[cluster_id_list[i]]->getName() << "    "; cout <<
      // _cluster_map[cluster_id_list[j]]->getName() << "   " << endl;
    }
...
void autoclusterMgr::printMacroCluster(Cluster* cluster_old, int& cluster_id)
{
  ...
  for (int i = 0; i < macro_vec.size(); i++) {
    ...
    output_file << endl;
    Cluster* cluster
        = new Cluster(++cluster_id, true, _network->pathName(macro_vec[i]));
    _cluster_map[cluster_id] = cluster;
  ...
  while (map_iter != _cluster_map.end()) {
    ...
    unordered_map<int, unsigned int> connection_map
        = map_iter->second->getOutputConnectionMap();
    ...
    while (iter != connection_map.end()) {
      if (iter->first != src_id) {
        ...
        output_file << _cluster_map[iter->first]->getName() << "   "
                    << iter->second << "   ";
      }
      iter++;
    }
                                     ...
                                     const char* file_name)
{
  ...
  output_file << "[INFO] Floorplan width: "
              << (_floorplan_ux - _floorplan_lx) / _dbu << endl;
  output_file << "[INFO] Floorplan height:  "
              << (_floorplan_uy - _floorplan_ly) / _dbu << endl;
  ...
  output_file << "[INFO] Num std cells: "
              << _logical_cluster_map[_network->topInstance()].num_inst << endl;
  output_file << "[INFO] Num macros: "
              << _logical_cluster_map[_network->topInstance()].num_macro
              << endl;
  output_file << "[INFO] Total area: "
              << _logical_cluster_map[_network->topInstance()].area << endl;
  output_file << "[INFO] Num buffers:  " << _num_buffer << endl;
  output_file << "[INFO] Buffer area:  " << _area_buffer << endl;
  output_file << endl;
  _logger->info(
      PAR, 403, "Number of Clusters created: {}", _cluster_list.size());
  ...
  while (map_iter != _cluster_map.end()) {
    ...
    if (!(connection_map.size() == 0
      ...
      bool flag = (src_id >= 1 && src_id <= 12)
                  || (_cluster_map[src_id]->getNumMacro() > 0);
      output_file << "Net_" << ++net_id << ":  " << endl;
      output_file << "source: " << map_iter->second->getName() << "   ";
      while (iter != connection_map.end()) {
        if (iter->first != src_id) {
          ...
          if (flag || (iter->first >= 1 && iter->first <= 12)
              || _cluster_map[iter->first]->getNumMacro() > 0) {
