...
namespace pin_alignment {
    ...
    bool PinAlignmentSingleCluster(shape_engine::Cluster* cluster, 
                                   std::unordered_map<std::string, std::pair<float, float> >& terminal_position,
                                   std::vector<block_placement::Net*>& nets, utl::Logger* logger, 
                                   float halo_width, int num_thread, int num_run, unsigned seed);
