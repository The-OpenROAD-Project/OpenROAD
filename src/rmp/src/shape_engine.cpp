    ...
    vector<pair<float, float> > TileMacro(vector<Macro> macros,  string cluster_name, float& final_area,
        ...
        while(remaining_run > 0) {
            ...
            for(int i = 0; i < run_thread; i++) {
                ...
                float temp_outline_height = outline_width * outline_height / temp_outline_width;
                SimulatedAnnealingCore* sa = new SimulatedAnnealingCore(macros, temp_outline_width, 
                        temp_outline_height, init_prob, rej_ratio, max_num_step, k, c, perturb_per_step,
                        alpha, pos_swap_prob,  neg_swap_prob, double_swap_prob, seed_list[i]);
