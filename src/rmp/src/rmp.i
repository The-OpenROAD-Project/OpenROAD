...
lext::Blif* create_blif(const char* hicell, const char* hiport, const char* locell, const char* loport, const int call_id=1){
  return new lext::Blif(getOpenRoad()->getLogger(), getOpenRoad()->getSta(), locell, loport, hicell, hiport, call_id);
}

void blif_add_instance(lext::Blif* blif_, const char* inst_){
  blif_->addReplaceableInstance(getOpenRoad()->getDb()->getChip()->getBlock()->findInst(inst_));
}

void blif_dump(lext::Blif* blif_, const char* file_name){
  blif_->writeBlif(file_name);
}

int blif_read(lext::Blif* blif_, const char* file_name){
