    ...
    sta::Units* units)
{
  ...
  if (!axis1_values) {
    logger_->error(utl::LEXT, 18, "axis 1 null cannot create ABC table");
  }
  ...
  if (!axis2_values) {
    logger_->error(utl::LEXT, 19, "axis 2 null cannot create ABC table");
  }
...
{
  if (!db_sta_) {
    logger_->error(utl::LEXT, 15, "Build called with null sta library");
  }
...
int AbcLibraryFactory::StaTimeUnitToAbcInt(sta::Unit* time_unit)
{
  ...
  if (abc_time_unit < 9 || abc_time_unit > 12) {
    logger_->error(
        utl::LEXT, 14, "Time unit {} outside abc range [1e-9, 1e-12]", scale);
  }
    ...
    const std::string& scale_abbreviation)
{
  ...
  logger_->error(
      utl::LEXT, 13, "Can't convert scale abbreviation {}", scale_abbreviation);
}
