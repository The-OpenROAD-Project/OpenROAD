...
``ord::report`` should be used instead of ‘puts’ so that all output is
logged.

Calls to the Tcl functions ord::warn and ord::error with a single
...
by ord::error are caught at the Tcl command level. Use the following
...
Debug messages are enabled with the tcl command: set_debug_level <tool>
<group> <level>
...
to pass in a Tool ID and message ID. For compatibility these are
defaulted to ‘UKN’ and ‘0000’ until they are updated.
