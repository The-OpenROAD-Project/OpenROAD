...
check "row bbox" {list [[$row getBBox] xMin] [[$row getBBox] yMin] [[$row getBBox] xMax] \
        [[$row getBBox] yMax]} "20140 22400 180120 25200" ;# Guess at the xMax value
...
check "wire techVia" {$wire getTechVia} "NULL"
...
# Return value is currently NULL, better to be an empty list
check "via tech via" {$via getTechVia} "NULL"
# Return value is currently NULL, better to be an empty list
check "via block via" {$via getBlockVia} "NULL"
