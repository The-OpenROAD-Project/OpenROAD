  ...
  if (splitPos > splitComp) {
    ...
    if (*bestDiff > splitPos - splitComp) {
      ...
      GifGetClosestPaletteColor(
          pPal, r, g, b, bestInd, bestDiff, treeRoot * 2 + 1);
    }
  } else {
    GifGetClosestPaletteColor(
        pPal, r, g, b, bestInd, bestDiff, treeRoot * 2 + 1);
                     ...
                     GifPalette* pal)
{
  ...
  // if the split is very unbalanced, split at the mean instead of the median to
  // preserve rare colors
  int splitUnbalance
  ...
  GifSplitPalette(
      image, subPixelsA, treeNode * 2, treeLevel + 1, buildForDither, pal);
...
// averaging the blocks at the bottom. This is known as the "median split"
// technique
                      ...
                      GifPalette* pPal)
{
  ...
  GifWriteCode(
      f, &stat, clearCode, codeSize);  // start with a fresh LZW dictionary
  ...
  for (uint32_t yy = 0; yy < height; ++yy) {
    for (uint32_t xx = 0; xx < width; ++xx) {
      ...
      // "worst possible mode" - no compression, every single code is followed
              ...
              bool dither = false)
{
  ...
  if (delay != 0) {
    ...
    fputc(1, writer->f);  // this is the Netscape 2.0 sub-block ID and it must
                          // be 1, otherwise some viewers error
                   ...
                   bool dither = false)
{
  ...
  GifPalette pal;
  GifMakePalette(
      (dither ? NULL : oldImage), image, width, height, bitDepth, dither, &pal);
  ...
  GifWriteLzwImage(
      writer->f, writer->oldImage, 0, 0, width, height, delay, &pal);
