# ORFS integration tests

A set of ORFS integration tests that runs in a few minutes suitable for
inclusion in the local fast regression testing workflow prior to creating a PR.

This directory is a separate Bazel workspace (`MODULE.bazel`) that depends on
OpenROAD via `local_path_override`.  The test cases are directly motivated by
OpenROAD integration testing concerns with flows that don't fit the mold of
ORFS nicely (idempotency, equivalence checking across stages, Chisel-generated
designs, Verilator simulation).

## Running tests

From the repo root:

    bazelisk run //:test             # lint + all tests (recommended before PRs)
...
1. Run `bazelisk run //gcd:gcd_update` to update RULES_JSON file for a design. This will build and run OpenROAD to generate a new RULES_JSON file and update the RULES_JSON source file.
...
Oneliner that runs tests, which builds all prerequisites in parallel, then update all the rules:
    ...
    bazelisk test ... && bazelisk query //... | grep _update\$ | xargs -n1 bazelisk run
...
Note that this update is needed infrequently because the compability between ORFS and OpenROAD only needs to be considered for `bazelisk test ...` tests.
...
First set up a folder with the necessary dependencies:
    ...
    bazelisk run //mock-array:Element_place_deps

This sets up a `tmp/mock-array/Element_place_deps/make` script that calls `make` on the ORFS setup. Since the place stage failed, build all place sub-stages up to the failing stage:

    tmp/mock-array/Element_place_deps/make do-place

Now create a standalone issue:

    tmp/mock-array/Element_place_deps/make global_place_skip_io_issue

The `WORK_HOME` is in `tmp/mock-array/Element_place_deps/_main`:
...
The `make` script, if `FLOW_HOME` is set, will use a local ORFS and OpenROAD built by CMake:
    ...
    $ tmp/mock-array/Element_place_deps/make print-FLOW_HOME print-OPENROAD_EXE
    [deleted]
...
More explicitly ORFS only:
    ...
    make --file=~/OpenROAD-flow-scripts/flow/Makefile -C tmp/mock-array/Element_place_deps/_main WORK_HOME=mock-array DESIGN_CONFIG=config.mk do-place
...
## Adding `tags = ["manual"]` to BUILD files

In OpenROAD, `bazelisk build ...` should not build ORFS targets, only OpenROAD binaries.

Since bazel-orfs also has build targets, builds in Bazel can build anything, not just executables, the policy is to mark non-test build targets as `tags = ["manual"]`.

To hunt down missing `tags = ["manual"]` run a query like:

    bazelisk query 'kind(".*", //mock-array/...) except attr(tags, "manual", //mock-array/...)'
...
`eqy_test` is used to run equivalence checks before and after an ORFS stage, such as before and after floorplan for mock-array. To run the test and keep all the files from the test and see interactive output, run:
    ...
    bazelisk test //mock-array:MockArray_4x4_eqy_test --test_output=streamed --sandbox_debug
...
If this fails, then it will output the line below. `eqy` uses a very, very large number of files and copying out these files to the bazel-testlogs folder for inspection takes some time:
    ...
    Copying 114462 files to bazel-testlogs/mock-array/MockArray_4x4_eqy_test/test.outputs for inspection.

If you just want the files needed to run a locally installed `eqy`, build all the files used in the run above by:

    bazelisk build //mock-array:MockArray_4x4_eqy_test

The files used to run the test are then in `bazel-bin/mock-array/`.

### TL;DR creating archive for standalone eqy test

Create `/tmp/issue.tar.gz`

    bazelisk build //gcd:gcd_eqy_synth_test
    tar --exclude='*oss_cad_suite*' -czvf /tmp/issue.tar.gz -C "$(bazelisk info bazel-bin)/gcd/gcd_eqy_synth_test.run.sh.runfiles" .

Reproduction instructions:

1. untar archive
2. cd _main
3. ~/oss-cad-suite/bin/eqy gcd/gcd_eqy_synth_test.eqy
...
This outputs:
    ...
    Target //gcd:gcd_eqy_synth_test up-to-date:
      bazel-bin/gcd/gcd_eqy_synth_test.run.sh

Copy all files needed to run the tests into a working folder `foo`:

    rsync -aL "$(bazelisk info bazel-bin)/gcd/gcd_eqy_synth_test.run.sh.runfiles/" foo/
...
Bazel sets up both binaries and data files to run the tests, to create a reportable standalone test. A report should include only the data files and not the actual binaries used locally to run the test:
    ...
    ./gcd
    ./gcd/gcd.v
    ./gcd/gcd_eqy_synth_test.run.sh
    ./gcd/gcd_eqy_synth.v
    ./gcd/gcd_eqy_synth_test.eqy

Bazel generated `bazel-bin/gcd/gcd_eqy_synth_test.run.sh` to runs the tests. Examine this script to look at that for clues as to how to run the tests standalone:

    $ head -n 5 gcd/gcd_eqy_synth_test.run.sh
    # !/bin/sh
    set -euo pipefail
    test_status=0
    (exec ../bazel-orfs++_repo_rules+oss_cad_suite/bin/eqy "$@" gcd/gcd_eqy_synth_test.eqy) || test_status=$?

What we want is:

    ../bazel-orfs++_repo_rules+oss_cad_suite/bin/eqy gcd/gcd_eqy_synth_test.eqy

Running this, we get:

<pre>$ ../bazel-orfs++_repo_rules+oss_cad_suite/bin/eqy gcd/gcd_eqy_synth_test.eqy
...
Let's say that `bazelisk test //gcd:eqy_synth_test` fails, first create a script that looks for the error string in the output, in this case a false positive `Failed to prove equivalence of partition gcd.req_rdy`.
...
set -euo pipefail
cp $1 gcd/
(bazelisk test //gcd:eqy_synth_test --test_timeout=30 --test_output=streamed || error=$?) | tee /dev/tty | grep "Failed to prove equivalence of partition gcd.req_rdy"
```
...
Next, run sv-bugpoint to whittle `gcd/gcd.v` down to a minimal test case:

    ~/sv-bugpoint/build/sv-bugpoint fail check.sh gcd/gcd.v
