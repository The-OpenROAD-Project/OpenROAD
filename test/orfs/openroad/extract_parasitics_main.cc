// SPDX-License-Identifier: BSD-3-Clause
// Copyright (c) 2026, The OpenROAD Authors
//
// Standalone extract_parasitics binary.
// Links only rcx + odb + utl.
//
// Usage:
//   extract_parasitics --read_db routed.odb [-ext_model_file model] [-write_spef out.spef]

#include <cstdlib>
#include <fstream>
#include <iostream>
#include <string>
#include <vector>

#include "odb/db.h"
#include "odb/defin.h"
#include "odb/lefin.h"
#include "rcx/ext.h"
#include "rcx/ext_options.h"
#include "utl/Logger.h"

namespace {

struct Options
{
  std::vector<std::string> read_lef;
  std::string read_def;
  std::string read_db;
  // extract options (same flags as TCL)
  std::string ext_model_file;
  int corner_cnt = 1;
  double max_res = 50.0;
  float coupling_threshold = 0.1;
  int context_depth = 5;
  int corner = -1;
  bool lef_rc = false;
  bool lef_res = false;
  bool no_merge_via_res = false;
  // output
  std::string write_spef;
};

void usage(const char* prog)
{
  std::cerr << "Usage: " << prog << " [options]\n"
            << "\n"
            << "I/O:\n"
            << "  --read_db FILE              Read ODB database\n"
            << "  --read_lef FILE             Read LEF (repeatable)\n"
            << "  --read_def FILE             Read DEF\n"
            << "  -write_spef FILE            Write SPEF output\n"
            << "\n"
            << "Extraction options (same flags as TCL):\n"
            << "  -ext_model_file FILE        Extraction model file\n"
            << "  -corner_cnt N               Number of corners\n"
            << "  -max_res N                  Max resistance (ohms)\n"
            << "  -coupling_threshold N       Coupling cap threshold (fF)\n"
            << "  -context_depth N            Context depth\n"
            << "  -corner N                   Corner index\n"
            << "  -lef_rc                     Use LEF RC values\n"
            << "  -lef_res                    Use LEF resistance\n"
            << "  -no_merge_via_res           Don't merge via resistance\n";
}

bool parse_args(int argc, char* argv[], Options& opts)
{
  for (int i = 1; i < argc; i++) {
    std::string arg = argv[i];
    auto next_str = [&]() -> std::string {
      if (i + 1 >= argc) {
        std::cerr << "Error: " << arg << " requires an argument\n";
        std::exit(1);
      }
      return argv[++i];
    };

    if (arg == "--read_db") {
      opts.read_db = next_str();
    } else if (arg == "--read_lef") {
      opts.read_lef.push_back(next_str());
    } else if (arg == "--read_def") {
      opts.read_def = next_str();
    } else if (arg == "-write_spef") {
      opts.write_spef = next_str();
    } else if (arg == "-ext_model_file") {
      opts.ext_model_file = next_str();
    } else if (arg == "-corner_cnt") {
      opts.corner_cnt = std::stoi(next_str());
    } else if (arg == "-max_res") {
      opts.max_res = std::stod(next_str());
    } else if (arg == "-coupling_threshold") {
      opts.coupling_threshold = std::stof(next_str());
    } else if (arg == "-context_depth") {
      opts.context_depth = std::stoi(next_str());
    } else if (arg == "-corner") {
      opts.corner = std::stoi(next_str());
    } else if (arg == "-lef_rc") {
      opts.lef_rc = true;
    } else if (arg == "-lef_res") {
      opts.lef_res = true;
    } else if (arg == "-no_merge_via_res") {
      opts.no_merge_via_res = true;
    } else if (arg == "-help" || arg == "--help" || arg == "-h") {
      usage(argv[0]);
      std::exit(0);
    } else {
      std::cerr << "Error: unknown argument: " << arg << "\n";
      usage(argv[0]);
      return false;
    }
  }

  bool has_db = !opts.read_db.empty();
  bool has_lef_def = !opts.read_lef.empty() || !opts.read_def.empty();
  if (!has_db && !has_lef_def) {
    std::cerr << "Error: specify --read_db or --read_lef/--read_def\n";
    return false;
  }
  return true;
}

}  // namespace

int main(int argc, char* argv[])
{
  Options opts;
  if (!parse_args(argc, argv, opts)) {
    return 1;
  }

  utl::Logger logger;
  auto* db = odb::dbDatabase::create();

  if (!opts.read_db.empty()) {
    std::ifstream f(opts.read_db, std::ios::binary);
    if (!f) {
      std::cerr << "Error: cannot open " << opts.read_db << "\n";
      return 1;
    }
    db->read(f);
  } else {
    odb::lefin lef_reader(db, &logger, false);
    for (const auto& lef_file : opts.read_lef) {
      const char* name = lef_file.c_str();
      if (!db->getTech()) {
        lef_reader.createTechAndLib(name, name, lef_file.c_str());
      } else {
        lef_reader.createLib(db->getTech(), name, lef_file.c_str());
      }
    }
    odb::defin def_reader(db, &logger);
    auto* tech = db->getTech();
    std::vector<odb::dbLib*> libs;
    for (auto* lib : db->getLibs()) {
      if (lib->getTech() == tech) {
        libs.push_back(lib);
      }
    }
    auto* chip = odb::dbChip::create(db, tech);
    def_reader.readChip(libs, opts.read_def.c_str(), chip);
  }

  auto* chip = db->getChip();
  if (!chip || !chip->getBlock()) {
    std::cerr << "Error: no design loaded\n";
    odb::dbDatabase::destroy(db);
    return 1;
  }

  rcx::Ext ext(db, &logger, "S");

  ExtractOptions ext_opts;
  ext_opts.ext_model_file = opts.ext_model_file.empty()
                                ? nullptr
                                : opts.ext_model_file.c_str();
  ext_opts.corner_cnt = opts.corner_cnt;
  ext_opts.max_res = opts.max_res;
  ext_opts.coupling_threshold = opts.coupling_threshold;
  ext_opts.context_depth = opts.context_depth;
  ext_opts.corner = opts.corner;
  ext_opts.lef_rc = opts.lef_rc;
  ext_opts.lef_res = opts.lef_res;
  ext_opts.no_merge_via_res = opts.no_merge_via_res;

  ext.extract(ext_opts);

  if (!opts.write_spef.empty()) {
    SpefOptions spef_opts;
    spef_opts.file = opts.write_spef.c_str();
    ext.write_spef(spef_opts);
  }

  odb::dbDatabase::destroy(db);
  return 0;
}
