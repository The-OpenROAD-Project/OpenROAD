...
//
// Standalone global_placement binary.
...
struct Options
{
  ...
  std::string write_db;
  // global_placement flags (same as TCL)
  float density = -1;
  int bin_grid_count = 0;
  bool skip_io = false;
  bool timing_driven = false;
  bool routability_driven = false;
...
int main(int argc, char* argv[])
{
  ...
  // Configure
  gpl::PlaceOptions place_opts;
  if (opts.density >= 0) {
    place_opts.density = opts.density;
  }
  if (opts.bin_grid_count > 0) {
    place_opts.binGridCntX = opts.bin_grid_count;
    place_opts.binGridCntY = opts.bin_grid_count;
  }
  if (opts.overflow >= 0) {
    place_opts.overflow = opts.overflow;
  }
  place_opts.skipIoMode = opts.skip_io;
  ...
  if (opts.incremental) {
    ...
    placer.doPlace(threads, place_opts);
  }
