...
//
// Standalone density_fill binary.
// Links only fin + odb + utl.
//
// Usage:
//   density_fill --read_db routed.odb --write_db filled.odb -rules rules.json
...
void usage(const char* prog)
{
  std::cerr << "Usage: " << prog << " [options]\n"
            ...
            << "\n"
            << "Required:\n"
            << "  -rules FILE             Density fill rules file\n";
}
...
bool parse_args(int argc, char* argv[], Options& opts)
{
  ...
  if (opts.rules.empty()) {
    std::cerr << "Error: -rules is required\n";
    return false;
  }
  return true;
}
...
int main(int argc, char* argv[])
{
  ...
  fin::Finale finale(db, &logger);
  odb::Rect fill_area = chip->getBlock()->getDieArea();
  finale.densityFill(opts.rules.c_str(), fill_area);
