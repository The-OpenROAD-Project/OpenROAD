...
//
// Standalone filler_placement binary.
...
bool parse_args(int argc, char* argv[], Options& opts)
{
  ...
  if (opts.master_patterns.empty()) {
    std::cerr << "Error: -masters is required\n";
    return false;
  }
  return true;
}
...
int main(int argc, char* argv[])
{
  ...
  if (filler_masters.empty()) {
    std::cerr << "Error: no masters matched the filler patterns\n";
    odb::dbDatabase::destroy(db);
    return 1;
  }

  dpl::Opendp opendp(db, &logger);
  opendp.fillerPlacement(filler_masters, opts.prefix.c_str(), opts.verbose);
