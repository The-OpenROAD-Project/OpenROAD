...
def _shim_target(name, pool = True, defines = []):
    return struct(name = name, pool = pool, defines = defines)

# Generated package config shims: interface targets over the merged
# include tree and (usually) the archive pool, a <pkg>_VERSION from
# deps-versions.cmake, aliases and extra variables. Packages with bespoke
# shapes (Boost's component loop, GTest's static imports) keep files in
# shims/; every package with a version key also gets a generated
# AnyNewerVersion <pkg>ConfigVersion.cmake.
_SHIM_SPECS = {
    "Eigen3": struct(
        version = "eigen",
        targets = [_shim_target("Eigen3::Eigen", pool = False)],
        aliases = {},
        variables = {
            "EIGEN3_INCLUDE_DIR": "${_OR_DEPS}/include",
            "EIGEN3_INCLUDE_DIRS": "${_OR_DEPS}/include",
        },
    ),
    "LEMON": struct(
        version = "coin_or_lemon",
        targets = [],
        aliases = {},
        variables = {
            "LEMON_INCLUDE_DIR": "${_OR_DEPS}/include",
            "LEMON_INCLUDE_DIRS": "${_OR_DEPS}/include",
            "LEMON_LIBRARIES": "openroad_deps::pool",
        },
    ),
    "absl": struct(
        version = None,
        targets = [
            _shim_target("absl::" + name)
            for name in ["city", "hash", "strings", "synchronization", "base", "time", "span"]
        ],
        aliases = {},
        variables = {},
    ),
    ...
    "ortools": struct(
        version = "or_tools",
        targets = [_shim_target("ortools::ortools")],
        aliases = {},
        variables = {},
    ),
    "spdlog": struct(
        version = "spdlog",
        targets = [_shim_target("spdlog::spdlog")],
        aliases = {"spdlog::spdlog_header_only": "spdlog::spdlog"},
        variables = {},
    ),
    "yaml-cpp": struct(
        version = "yaml_cpp",
        targets = [_shim_target("yaml-cpp")],
        aliases = {"yaml-cpp::yaml-cpp": "yaml-cpp"},
        variables = {},
    ),
}

# File-based shims that still want a generated ConfigVersion.cmake.
_SHIM_EXTRA_VERSIONS = {
    "Boost": "boost",
    "GTest": "googletest",
}

def _shim_config_cmake(pkg, spec):
    lines = [
        "# Generated by //cmake-deps (_SHIM_SPECS in defs.bzl).",
        "",
        'include("${CMAKE_CURRENT_LIST_DIR}/../openroad_deps/deps-pool.cmake")',
    ]
    if spec.version:
        lines.append('include("${CMAKE_CURRENT_LIST_DIR}/../openroad_deps/deps-versions.cmake")')
        lines.append("")
        lines.append('set({}_VERSION "${{OPENROAD_DEPS_VERSION_{}}}")'.format(pkg, spec.version))
    for var, value in spec.variables.items():
        lines.append('set({} "{}")'.format(var, value))
    for target in spec.targets:
        lines += [
            "",
            "if(NOT TARGET {})".format(target.name),
            "  add_library({} INTERFACE IMPORTED GLOBAL)".format(target.name),
            "  set_target_properties({} PROPERTIES".format(target.name),
            '    INTERFACE_INCLUDE_DIRECTORIES "${_OR_DEPS}/include"',
        ]
        if target.pool:
            lines.append("    INTERFACE_LINK_LIBRARIES openroad_deps::pool")
        if target.defines:
            lines.append("    INTERFACE_COMPILE_DEFINITIONS \"{}\"".format(
                ";".join(target.defines),
            ))
        lines += ["  )", "endif()"]
    for alias, target in spec.aliases.items():
        lines += [
            "",
            "if(NOT TARGET {})".format(alias),
            "  add_library({} ALIAS {})".format(alias, target),
            "endif()",
        ]
    return "\n".join(lines) + "\n"

def _shim_config_version_cmake(version):
    return "\n".join([
        "# Generated by //cmake-deps. AnyNewerVersion semantics.",
        "",
        'include("${CMAKE_CURRENT_LIST_DIR}/../openroad_deps/deps-versions.cmake")',
        "",
        'set(PACKAGE_VERSION "${OPENROAD_DEPS_VERSION_%s}")' % version,
...
def _shim_writes():
    writes = []
    for pkg, spec in _SHIM_SPECS.items():
        writes.append(struct(
            dest = "lib/cmake/{}/{}Config.cmake".format(pkg, pkg),
            content = _shim_config_cmake(pkg, spec),
            executable = False,
        ))
    versions = dict(_SHIM_EXTRA_VERSIONS)
    versions.update({
        pkg: spec.version
        for pkg, spec in _SHIM_SPECS.items()
        if spec.version
    ...
    for pkg, version in versions.items():
        writes.append(struct(
            dest = "lib/cmake/{}/{}ConfigVersion.cmake".format(pkg, pkg),
            content = _shim_config_version_cmake(version),
            executable = False,
        ))
    return writes
