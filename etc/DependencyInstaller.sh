...
_install_bazel() {
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        ...
        # One piece of it still reaches outside bazel: naja's verilog scanner
        # and parser are generated by the host bison/flex/m4, through
        # naja-verilog's own Starlark rules. Without them the failure surfaces
        # inside a bazel fetch as an unhelpful repository-rule error, so
        # install them alongside bazel itself.
        #
        # Nothing else is needed. cmake, make and pkg-config were required
        # while oneTBB was a rules_foreign_cc cmake() target and are not now
        # that it comes from the BCR module; python3-dev was required by naja's
        # pyloader, which is no longer linked into the binary. See the
        # kepler-formal overrides in MODULE.bazel.
        if _command_exists "apt-get"; then
