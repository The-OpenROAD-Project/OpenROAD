...
def _swig_vec_to_list(vec):
    """Convert SWIG std::vector to Python list using index access.

    Python iteration (for x in vec / list(vec)) on SWIG vectors triggers
    a swig::stop_iteration C++ exception during garbage collection that
    aborts the process.  Index-based access avoids this.
    """
    return [vec[i] for i in range(len(vec))]


def _pin_name(iterm, bterm):
    """Get hierarchical pin name from ODB iterm/bterm pair."""
    if iterm:
...
def extract_timing_data(
    timing: Timing, design_obj: Design, design_name: str, platform: str
) -> dict:
    ...
    # Endpoint slacks — filter out unconstrained (infinite slack)
    endpoint_slacks = []
    unconstrained_count = 0
    raw_slacks = timing.getEndpointSlacks(Timing.Max)
    for i in range(len(raw_slacks)):
        entry = raw_slacks[i]
        slack = entry.slack
        if timing.isTimeInf(slack) or timing.isTimeInf(-slack):
            unconstrained_count += 1
        else:
            name = _pin_name(entry.iterm, entry.bterm)
            endpoint_slacks.append({"name": name, "slack": slack})

    # Timing paths via C++ API
    raw_paths = timing.getTimingPaths(Timing.Max, endpoint_count)
    setup_paths = []
    for i in range(len(raw_paths)):
        tp = raw_paths[i]
        arcs = []
        for j in range(len(tp.arcs)):
            arc = tp.arcs[j]
            is_net = arc.master is None
            cell_name = "" if is_net else arc.master.getName()
            arcs.append(
                {
                    "from": _pin_name(arc.from_iterm, arc.from_bterm),
                    "to": _pin_name(arc.to_iterm, arc.to_bterm),
                    "cell": cell_name,
                    "delay": arc.delay,
                    "slew": arc.slew,
                    "load": arc.load,
                    "fanout": arc.fanout,
                    "rising": arc.is_rising,
                    "net": is_net,
                }
            )
        setup_paths.append(
            {
                "slack": tp.slack,
                "path_delay": tp.path_delay,
                "arrival": tp.arrival,
                "required": tp.required,
                "skew": tp.skew,
                "logic_delay": tp.logic_delay,
                "logic_depth": tp.logic_depth,
                "fanout": tp.fanout,
                "start": _pin_name(tp.start_iterm, tp.start_bterm),
                "end": _pin_name(tp.end_iterm, tp.end_bterm),
                "start_clk": tp.start_clock,
                "end_clk": tp.end_clock,
                "group": tp.path_group,
                "arcs": arcs,
            }
        )

    # Path groups from the paths we found
    path_groups = sorted(set(p["group"] for p in setup_paths if p["group"]))

    # Clock domains via C++ API
    clocks = []
    raw_clocks = timing.getClockInfo()
    for i in range(len(raw_clocks)):
        clk = raw_clocks[i]
        sources = []
        for it in _swig_vec_to_list(clk.source_iterms):
            sources.append(_pin_name(it, None))
        for bt in _swig_vec_to_list(clk.source_bterms):
            sources.append(_pin_name(None, bt))
        clocks.append(
            {
                "name": clk.name,
                "period": round(clk.period, 4),
                "waveform": _swig_vec_to_list(clk.waveform),
                "sources": sources,
            }
        )
...
def generate_markdown(data: dict, path: str):
    ...
    lines.append("|---|---|---|")
    lines.append(
        f"| WNS | `{data['setup_wns']:.4f} ns` | " f"`{data['hold_wns']:.4f} ns` |"
    )
    lines.append(
        f"| TNS | `{data['setup_tns']:.4f} ns` | " f"`{data['hold_tns']:.4f} ns` |"
    )
    ...
    if data["clocks"]:
        ...
        for clk in data["clocks"]:
            ...
            lines.append(f"| `{clk['name']}` | `{clk['period']}` | " f"`{sources}` |")
        lines.append("\n</details>\n")
    ...
    if failing:
        shown = failing[:20]
        lines.append(
            f"<details><summary>Top Failing Paths " f"({len(failing)})</summary>\n"
        )
        ...
        for p in shown:
            ...
            lines.append(
                f"| `{p['slack']:.4f}` | `{start}` | " f"`{end}` | {p['group']} |"
            )
        if len(failing) > 20:
    ...
    if cells:
        top_cells = sorted(cells.items(), key=lambda x: -x[1]["delay"])[:10]
        lines.append(
            "<details><summary>Cell Type Breakdown " "(top 10 by delay)</summary>\n"
        )
...
def main():
    ...
    if not odb_file or not sdc_file:
        print(
            "ERROR: ODB_FILE and SDC_FILE environment variables required",
            file=sys.stderr,
        )
        sys.exit(1)
